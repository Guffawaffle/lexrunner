...
describe("Merge Order Computation", () => {
  it("computes linear order for tiny plan", () => {
    ...
    expect(levels).toEqual([["feat-a"], ["feat-b"], ["feat-c"]]);
  });

  it("computes parallel groups correctly", () => {
    const planContent = fs.readFileSync(
      path.join(__dirname, "fixtures/plan.parallel.json"),
      "utf-8"
    );
    const plan = loadPlan(planContent);
    const levels = computeMergeOrder(plan);
    ...
    expect(levels).toEqual([["feat-a", "feat-b", "feat-f"], ["feat-c", "feat-d"], ["feat-e"]]);
  });
  ...
  it("detects unknown dependencies", () => {
    const planContent = fs.readFileSync(
      path.join(__dirname, "fixtures/plan.bad-unknown-dep.json"),
      "utf-8"
    );
    const plan = loadPlan(planContent);
