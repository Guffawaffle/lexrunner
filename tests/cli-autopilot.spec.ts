...
describe("CLI autopilot command", () => {
    ...
    const plan = {
      ...
      items: [
        ...
        },
        {
          name: "item-2",
          deps: ["item-1"],
          gates: [{ name: "test", run: "npm test", env: {} }],
        },
      ],
    };
  ...
  it("should run level 0 autopilot successfully", () => {
    const output = execSync(`npm run cli -- autopilot ${planFile} --level 0`, {
      encoding: "utf-8",
      cwd: process.cwd(),
    });
  ...
  it("should run level 0 autopilot with JSON output", () => {
    const output = execSync(`npm run cli -- autopilot ${planFile} --level 0 --json 2>/dev/null`, {
      encoding: "utf-8",
      cwd: process.cwd(),
    });
  ...
  it("should fail when writing to role=example profile", () => {
    ...
    try {
      execSync(`npm run cli -- autopilot ${planFile} --level 1 --profile-dir ${exampleProfile}`, {
        encoding: "utf-8",
        cwd: process.cwd(),
        stdio: "pipe",
      });
  ...
  it("should support --plan flag", () => {
    const output = execSync(`npm run cli -- autopilot --plan ${planFile} --level 0`, {
      encoding: "utf-8",
      cwd: process.cwd(),
    });
  ...
  it("should error on invalid level", () => {
    try {
      execSync(`npm run cli -- autopilot ${planFile} --level 99 2>&1`, {
        encoding: "utf-8",
        cwd: process.cwd(),
      });
      expect.fail("Should have thrown an error");
  ...
  it("should error when plan file is missing", () => {
    try {
      execSync(`npm run cli -- autopilot --level 0 2>&1`, {
        encoding: "utf-8",
        cwd: process.cwd(),
      });
      expect.fail("Should have thrown an error");
