...
describe("MCP Contract Tests", () => {
	...
	describe("Parameter validation schemas", () => {
		it("should validate PlanCreateArgs schema correctly", () => {
			...
			expect(() =>
				PlanCreateArgs.parse({ outDir: "/tmp" })
			).not.toThrow();
			expect(() =>
				PlanCreateArgs.parse({ json: true, outDir: "/tmp" })
			).not.toThrow();

			// Valid arguments - GitHub auto-discovery
			expect(() =>
				PlanCreateArgs.parse({ fromGithub: true })
			).not.toThrow();
			expect(() =>
			...
			expect(() =>
				PlanCreateArgs.parse({ fromGithub: true, includeDrafts: false })
			).not.toThrow();
			expect(() =>
				PlanCreateArgs.parse({
					fromGithub: true,
			...
			expect(() =>
				PlanCreateArgs.parse({ fromGithub: true, maxWorkers: 4 })
			).not.toThrow();
			expect(() =>
				PlanCreateArgs.parse({ fromGithub: true, target: "develop" })
			).not.toThrow();
			...
			expect(() =>
				PlanCreateArgs.parse({ fromGithub: "true" })
			).toThrow(); // Should be boolean
			expect(() =>
				PlanCreateArgs.parse({ labels: "bug,feature" })
			).toThrow(); // Should be array
			expect(() =>
				PlanCreateArgs.parse({ excludePRs: "123,456" })
			).toThrow(); // Should be array of numbers
			expect(() =>
				PlanCreateArgs.parse({ excludePRs: [123, "456"] })
			).toThrow(); // Should be numbers, not strings
			expect(() => PlanCreateArgs.parse({ maxWorkers: "4" })).toThrow(); // Should be number
			expect(() =>
				PlanCreateArgs.parse({ requiredGates: "lint,test" })
			).toThrow(); // Should be array
		});
		...
		it("should validate GatesRunArgs schema correctly", () => {
			...
			expect(() =>
				GatesRunArgs.parse({ onlyItem: "item1" })
			).not.toThrow();
			expect(() =>
				GatesRunArgs.parse({ onlyGate: "test" })
			).not.toThrow();
			expect(() => GatesRunArgs.parse({ outDir: "/tmp" })).not.toThrow();
			expect(() =>
				GatesRunArgs.parse({ onlyItem: "item1", onlyGate: "test" })
			).not.toThrow();
			expect(() =>
				GatesRunArgs.parse({ planFile: "/path/to/plan.json" })
			).not.toThrow();
			expect(() =>
				GatesRunArgs.parse({
	...
	describe("Schema type compliance", () => {
		...
		it("should parse GitHub auto-discovery parameters with correct types", () => {
			...
			expect(githubArgs.requiredGates).toEqual([
				"lint",
				"test",
				"security",
			]);
