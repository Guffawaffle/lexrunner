...
describe("Conflict Predictor Integration", () => {
  ...
  it("should identify conflicts and recommend batches", async () => {
    ...
    const hasInternalConflict = report.conflictGraph.edges.some(
      (edge) => firstBatchSet.has(edge.from) && firstBatchSet.has(edge.to)
    );
  ...
  it("should handle batch 1 scenario from issue (PRs 166, 167, 168)", async () => {
    ...
    const has167Sequential = report.misBatches.some(
      (batch, idx) => idx > 0 && batch.prs.includes("167")
    );
  ...
  it("should include timestamp in report", async () => {
    const prs: PRWithFiles[] = [{ number: 1, files: ["a.ts"] }];
