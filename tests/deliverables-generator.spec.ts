...
} from "../src/orchestration/deliverablesGenerator";
...
describe("Deliverables Generator", () => {
	...
	describe("computePlanHash", () => {
		...
		it("should use canonical JSON (key ordering)", () => {
			...
			const planA = JSON.parse(JSON.stringify({
				target: "main",
				schemaVersion: "1.0.0",
				items: [{ name: "test", deps: [], gates: [] }]
			}));

			const planB = JSON.parse(JSON.stringify({
				schemaVersion: "1.0.0",
	...
	describe("generateDeliverables", () => {
		...
		it("should generate toolchain manifest with required tools", async () => {
			...
			const manifestContent = fs.readFileSync(path.join(outputDir, "toolchain-manifest.json"), "utf-8");
