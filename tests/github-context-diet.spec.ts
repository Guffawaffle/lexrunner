...
`;
			...
			expect(result.metrics.totalSize).toBe(
				result.metrics.prMetadataSize! +
				result.metrics.diffSize! +
				result.metrics.symbolMapSize!
			);
		});
...
+++ b/src/large.ts
@@ -1,50 +1,50 @@
${Array(50).fill(0).map((_, i) => ` line ${i + 1}`).join('\n')}
`;
...
+++ b/src/large.ts
@@ -1,1000 +1,1000 @@
${Array(1000).fill(0).map((_, i) => ` line ${i + 1}`).join('\n')}
`;
...
`;
			const files = [
				{ path: 'src/test.ts', content: 'export function test() { return 1; }' }
			];
