 ...
 */
export function fail(gateName: string, options: {
  exitCode?: number;
  stderr?: string;
  duration?: number;
  attempts?: number;
 ...
 */
export function someFail(config: {
  pass: string[];
  fail: string[];
}): GateResult[] {
  return [
    ...config.pass.map(name => pass(name)),
    ...config.fail.map(name => fail(name))
  ];
}
 ...
 */
export function lintResult(options: {
  errors?: number;
  warnings?: number;
  duration?: number;
} = {}): GateResult {
