  ...
  for (let layer = 0; layer < layers && nodeIndex < nodes; layer++) {
    ...
    for (let i = 0; i < actualItems; i++) {
      ...
      if (layer > 0) {
        ...
        const prevLayerStart = calculatePreviousLayerStart(
          layer,
          layers,
          nodes,
          nodeIndex,
        ...
        for (
          let d = 0;
          d < depsCount && prevLayerStart + d >= 0 && prevLayerStart + d < nodeIndex;
          d++
        ) {
...
function calculatePreviousLayerStart(
  ...
  return (
    nodeIndex -
    actualItems -
    Math.max(1, Math.ceil(((nodes / layers) * (layers - layer + 1)) / layers))
  );
}
