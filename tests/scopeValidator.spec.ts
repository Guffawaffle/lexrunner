...
import { validateEditScope, analyzeFileScope, ScopeValidationError, type EditPlan } from "../src/planner/scopeValidator.js";
  ...
  describe("validateEditScope", () => {
    ...
    it("should fail validation for undeclared function modifications", async () => {
      ...
      try {
        ...
        expect(scopeError.violations?.some(v => v.message.includes('anotherFunction'))).toBe(true);
      }
    });
    ...
    it("should fail validation for undeclared global writes", async () => {
      ...
      try {
        ...
        expect(scopeError.violations?.some(v => v.message.includes('window.DEBUG_MODE'))).toBe(true);
      }
    });
    ...
    it("should handle complex AMD module case from issue", async () => {
      ...
      try {
        ...
        expect(scopeError.violations?.some(v => 
          v.type === 'undeclared_function' && v.message.includes('auditLog')
        )).toBe(true);
        
        // Should detect undeclared global write
        expect(scopeError.violations?.some(v => 
          v.type === 'global_write' && v.message.includes('window.DEBUG_MODE')
        )).toBe(true);
