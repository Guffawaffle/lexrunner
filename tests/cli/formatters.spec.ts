...
describe("CLI Formatters", () => {
    ...
    it("should format status table with results", () => {
      const results = new Map([
        [
          ...
          {
            status: "pass",
            gates: [{ status: "pass", gate: "test", duration: 100 }],
        ...
        [
          ...
          {
            status: "fail",
            gates: [{ status: "fail", gate: "lint", duration: 50 }],
  ...
  describe("formatMarkdown", () => {
    it("should format single section", () => {
      const sections: MarkdownSection[] = [{ heading: "Test", content: "Content here" }];
  ...
  describe("formatCSV", () => {
    ...
    it("should escape values correctly", () => {
      const data = [{ name: "test, with comma", value: "ok" }];
      const result = formatCSV(data);
  ...
  describe("formatQueryResult", () => {
    ...
    it("should format table output (default)", () => {
      const result = {
        items: [{ name: "item1", level: 1, deps: ["dep1"], gates: [{ name: "test" }] }],
        count: 1,
        query: "test query",
      };
      const output = formatQueryResult(result, "table");
    ...
    it("should show dependents if present", () => {
      const result = {
        items: [{ name: "item1", level: 1, deps: [], dependents: ["item2", "item3"], gates: [] }],
        count: 1,
        query: "test",
      };
      const output = formatQueryResult(result, "table");
  ...
  describe("Edge cases", () => {
    it("should handle very long strings in tables", () => {
      const data = [{ name: "a".repeat(100), value: "short" }];
      const result = formatTable(data);
