...
} from "../src/runs/statusBuilder.js";
...
describe("Run Manager", () => {
	...
	describe("startRun", () => {
		...
		it("should include optional task parameter", () => {
			...
			expect(runState.task).toBe(
				"Merge all feature branches for release"
			);
		});
...
describe("Integration: startRun → getStatus", () => {
	...
	it("should start run and verify state with getStatus", () => {
		...
		expect(status.nextOptions).toEqual(
			startResult.initialStatus.nextOptions
		);
	});
