...
describe("Tier Suggestion Heuristics", () => {
  ...
  const createItem = (name: string, deps: string[] = [], gates: Gate[] = []): PlanItem => ({
    name,
    deps,
    gates,
  });
  ...
  describe("isLintOnly", () => {
    ...
    it("should return true for multiple deterministic gates", () => {
      const item = createItem("check-task", [], [createGate("lint"), createGate("typecheck")]);
      expect(isLintOnly(item)).toBe(true);
    });

    it("should return false if any non-deterministic gate", () => {
      const item = createItem("mixed-task", [], [createGate("lint"), createGate("test")]);
  ...
  describe("suggestTier", () => {
    ...
    it("should prioritize senior indicators over junior", () => {
      ...
      const item = createItem("complex-lint", ["a", "b", "c", "d"], [createGate("lint")]);
      expect(suggestTier(item)).toBe("senior");
    });
  });
