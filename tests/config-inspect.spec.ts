...
`
			...
			const output1 = execSync(`node ${cliPath} config:inspect --json`, {
				encoding: "utf8",
			});
			const output2 = execSync(`node ${cliPath} config:inspect --json`, {
				encoding: "utf8",
			});
...
`
			...
			const output = execSync(`node ${cliPath} config:inspect --json`, {
				encoding: "utf8",
			});
...
`
		...
		it("should indicate non-existent config files", (ctx) => {
			...
			const stackSource = result.sources.find(
				(s: any) => s.file === "stack.yml"
			);
...
`
			...
			const output = execSync(`node ${cliPath} config:inspect`, {
				encoding: "utf8",
			});
