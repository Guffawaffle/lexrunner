/**
 * Tests for preflight conflict detection
 */

import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { detectPreflightConflicts, skipPreflightDetection } from '../src/weave/preflightConflicts.js';
...
describe('Preflight Conflict Detection', () => {
	...
	describe('detectPreflightConflicts', () => {
		...
		it('should detect conflicts when branches modify the same file', async () => {
			...
			await execa('git', ['add', '.'], { cwd: tempDir });
			await execa('git', ['commit', '-m', 'Modify file in branch 1'], { cwd: tempDir });
			...
			await execa('git', ['add', '.'], { cwd: tempDir });
			await execa('git', ['commit', '-m', 'Modify file in branch 2'], { cwd: tempDir });
		...
		it('should handle missing branches gracefully', async () => {
			const plan: Plan = {
				...
				target: 'main',
				items: [
					{ name: 'nonexistent-branch', deps: [], gates: [] },
				],
			};
		...
		it('should return merge base information when available', async () => {
			...
			await execa('git', ['add', '.'], { cwd: tempDir });
			await execa('git', ['commit', '-m', 'Add new file'], { cwd: tempDir });
