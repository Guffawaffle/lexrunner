...
describe("init command with --json flag", () => {
	...
	it("should create workspace files successfully", async () => {
		...
		expect(await fs.pathExists(path.join(profileDir, "intent.md"))).toBe(
			true
		);
		expect(await fs.pathExists(path.join(profileDir, "scope.yml"))).toBe(
			true
		);
		expect(await fs.pathExists(path.join(profileDir, "deps.yml"))).toBe(
			true
		);
		expect(await fs.pathExists(path.join(profileDir, "gates.yml"))).toBe(
			true
		);
		expect(
			await fs.pathExists(path.join(profileDir, "profile.yml"))
		).toBe(true);
	});
