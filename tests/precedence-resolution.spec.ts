...
function getLexPackageJson(): {
  ...
  const lexPkgPath = path.join(process.cwd(), "node_modules/@smartergpt/lex/package.json");
...
describe("Precedence Resolution Tests", () => {
  ...
  describe("4-Level Precedence Chain", () => {
    describe("Level 1: LEX_PROMPTS_DIR (highest precedence)", () => {
      it("should use LEX_PROMPTS_DIR when set, ignoring all other sources", () => {
        ...
        fs.writeFileSync(path.join(fixture.envDir, "test.md"), "# ENV Level");
        fs.writeFileSync(path.join(fixture.localOverlayDir, "test.md"), "# Local Overlay Level");
        fs.writeFileSync(path.join(fixture.trackedDir, "test.md"), "# Tracked Level");
      ...
      it("should throw descriptive error when LEX_PROMPTS_DIR points to non-existent directory", () => {
        ...
        expect(() => resolvePromptsDir(fixture.testDir)).toThrow(PromptsResolverError);
    ...
    describe("Level 2: .smartergpt.local/prompts (local overlay)", () => {
      it("should use local overlay when no LEX_PROMPTS_DIR is set", () => {
        ...
        fs.writeFileSync(path.join(fixture.localOverlayDir, "test.md"), "# Local Overlay");
        fs.writeFileSync(path.join(fixture.trackedDir, "test.md"), "# Tracked");
    ...
    describe("Level 3: .smartergpt/prompts (tracked canon)", () => {
      it("should use tracked prompts when no higher precedence sources exist", () => {
        ...
        fs.mkdirSync(fixture.trackedDir, { recursive: true });
        fs.writeFileSync(path.join(fixture.trackedDir, "test.md"), "# Tracked Canon");
      ...
      it("should prefer local overlay over tracked when both exist", () => {
        ...
        fs.writeFileSync(path.join(fixture.localOverlayDir, "test.md"), "# Local Overlay Wins");
        fs.writeFileSync(path.join(fixture.trackedDir, "test.md"), "# Tracked Should Lose");
    ...
    describe("Level 4: @smartergpt/lex package (fallback)", () => {
      it("should fall back to package when no local sources exist", () => {
        ...
        try {
          ...
          expect(["@smartergpt/lex/prompts", "@smartergpt/lex/canon/prompts"]).toContain(
            resolved.source
          );
        } catch (error) {
          ...
          if (error instanceof PromptsResolverError) {
            ...
            expect(error.message).toContain("Prompts directory not found");
  ...
  describe("Package Fallback Tests", () => {
    ...
    it("should skip local overlay if empty directory exists but has no prompts", () => {
      ...
      fs.mkdirSync(fixture.trackedDir, { recursive: true });
      fs.writeFileSync(path.join(fixture.trackedDir, "test.md"), "# Tracked Content");
  ...
  describe("Error Message Chain Tests", () => {
    ...
    it("should include source in prompt-not-found error", () => {
      ...
      fs.mkdirSync(fixture.trackedDir, { recursive: true });
      fs.writeFileSync(path.join(fixture.trackedDir, "exists.md"), "# Exists");
      ...
      try {
        ...
        expect(error).toBeInstanceOf(PromptsResolverError);
        if (error instanceof PromptsResolverError) {
          expect(error.message).toContain("Prompt not found: nonexistent");
          expect(error.message).toContain(".smartergpt/prompts");
        }
      }
    });
  ...
  describe("Cross-Repository Integration Tests", () => {
    it("should support cross-repo prompt consumption via LEX_PROMPTS_DIR", () => {
      ...
      const externalPromptsDir = path.join(externalRepoDir, ".smartergpt/prompts");
...
`;
      fs.writeFileSync(path.join(externalPromptsDir, "external-prompt.md"), externalPrompt);
    ...
    it("should support symlinked prompts directory for shared prompts", () => {
      ...
      const consumerLocalOverlay = path.join(consumerDir, ".smartergpt.local/prompts");
  ...
  it("should produce consistent error message format for missing prompts", () => {
    try {
      ...
      if (error instanceof PromptsResolverError) {
        ...
        const expectedPattern = /Prompts directory not found\. Expected one of:/;
        ...
        expect(error.message).toContain("- " + path.join(testDir, ".smartergpt.local/prompts"));
        expect(error.message).toContain("- " + path.join(testDir, ".smartergpt/prompts"));
        expect(error.message).toContain("- @smartergpt/lex/prompts");
        expect(error.message).toContain("- @smartergpt/lex/canon/prompts");
      }
    }
