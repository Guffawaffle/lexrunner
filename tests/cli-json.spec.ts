...
`
  ...
  describe("merge-order --json", () => {
    ...
    it("should output deterministic merge order JSON", (ctx) => {
      if (skipIfCliNotBuilt({ skip: ctx.skip })) return;
      const output1 = execSync(`node ${cliPath} merge-order plan.json --json`, {
        encoding: "utf8",
      });
      const output2 = execSync(`node ${cliPath} merge-order plan.json --json`, {
        encoding: "utf8",
      });
  ...
  describe("execute --json", () => {
    ...
    it("should output deterministic execution results in dry-run mode", (ctx) => {
      if (skipIfCliNotBuilt({ skip: ctx.skip })) return;
      const output1 = execSync(`node ${cliPath} execute plan.json --dry-run --json`, {
        encoding: "utf8",
      });
      const output2 = execSync(`node ${cliPath} execute plan.json --dry-run --json`, {
        encoding: "utf8",
      });
