...
describe("WeaveStateMachine", () => {
  describe("State Transitions", () => {
    it("should start in idle state", () => {
      const context = createWeaveContext({ target: "main", items: [] }, [], "test-hash");
  ...
  describe("State Transition Coverage", () => {
    it("should have all transitions defined", () => {
      ...
      const hasComplete = STATE_TRANSITIONS.some((t) => t.to === WeaveState.COMPLETED);
      const hasPause = STATE_TRANSITIONS.some((t) => t.event === WeaveEvent.PAUSE);
      const hasResume = STATE_TRANSITIONS.some((t) => t.event === WeaveEvent.RESUME);
