...
describe("assignAgentsToBatch", () => {
  ...
  it("should assign agents successfully", async () => {
    const mockAssign = vi
      .fn()
      .mockResolvedValue({ url: "https://github.com/test/repo/issues/156" });
    const issues = [156, 157];

    const result = await assignAgentsToBatch(issues, { repo: "test/repo", stagger: 0 }, mockAssign);
  ...
  it("should handle rate limiting with retry", async () => {
    ...
    const promise = assignAgentsToBatch([156], { repo: "test/repo", stagger: 0 }, mockAssign);
  ...
  it("should skip already assigned issues", async () => {
    ...
    const result = await assignAgentsToBatch([156], { repo: "test/repo", stagger: 0 }, mockAssign);
