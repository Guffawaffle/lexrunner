...
describe("Vacuum-Ready Prompt Schema", () => {
  ...
  describe("ScopeClosure", () => {
    it("should validate scope closure with all fields", () => {
      ...
      expect(result.description).toBe("Review code in src and tests directories");
    });
  ...
  describe("ToolAnchoring", () => {
    it("should validate tool anchoring with all fields", () => {
      ...
      expect(result.allowedTools).toEqual(["grep_search", "read_file", "get_errors"]);
  ...
  describe("PolicyAttachment", () => {
    it("should validate policy attachment with all fields", () => {
      const policy = {
        guardrails: ["Read-only operations", "No external API calls", "Stay within allowed paths"],
        constraints: ["Maximum 50 files", "No files larger than 1MB"],
        escalationRules: ["Escalate on security concerns", "Escalate on ambiguous requirements"],
      };
  ...
  describe("TaskAtomicity", () => {
    it("should validate task atomicity with all fields", () => {
      ...
      expect(result.deliverable).toBe("Code review decision with rationale");
    ...
    it("should validate all deliverable types", () => {
      const validTypes = ["frame", "report", "decision", "artifact", "analysis"];
  ...
  describe("OutcomeCheckability", () => {
    it("should validate outcome checkability with all fields", () => {
      const outcome = {
        ...
        failureCriteria: ["Decision is missing", "No rationale provided"],
      ...
      expect(result.acceptanceTest).toBe("Validate output matches frame schema");
    });
  ...
  describe("Example Prompts - Validation", () => {
    it("should validate code review prompt example", () => {
      const codeReviewPrompt = {
        ...
        policyAttached: {
          guardrails: ["Read-only access", "No modification of files", "Respect file size limits"],
        },
        taskAtomic: {
        ...
        outcomeCheckable: {
          successCriteria: ["Frame contains decision field", "Frame contains rationale"],
        },
      };
    ...
    it("should validate refactoring task prompt example", () => {
      const refactoringPrompt = {
        ...
        policyAttached: {
          guardrails: ["Preserve existing API", "Maintain test coverage", "No breaking changes"],
    ...
    it("should validate testing task prompt example", () => {
      const testingPrompt = {
        ...
        outcomeCheckable: {
          ...
          failureCriteria: ["Tests do not compile", "Coverage below threshold"],
