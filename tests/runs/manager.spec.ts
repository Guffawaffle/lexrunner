...
describe("RunManager", () => {
  ...
  describe("completeRunWithFrame (AX-005)", () => {
    it("should emit and persist Frame on run completion", async () => {
      ...
      const { runState, frameResult } = await manager.completeRunWithFrame(run.runId, "success", [
        "Deploy to staging",
        "Run e2e tests",
      ]);
      ...
      const decisions = await manager.getDecisions(run.runId);
      const frameEmittedDecision = decisions.find((d) => d.type === "frame_emitted");
    ...
    it("should emit partial Frame with partial outcome", async () => {
      ...
      const { runState, frameResult } = await manager.completeRunWithFrame(run.runId, "partial", [
    ...
    it("should include artifacts in Frame metadata", async () => {
      ...
      expect(frameResult.frame!.metadata?.artifacts).toEqual(["/tmp/report.json", "/tmp/logs.txt"]);
    });
  });
});
