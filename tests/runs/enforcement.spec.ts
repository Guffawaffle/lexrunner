...
describe("Enforcement Module", () => {
  ...
  describe("detectGitViolation", () => {
    it("should detect git merge", () => {
      expect(detectGitViolation("git merge main")).toBe(ViolationType.DIRECT_MERGE);
    });

    it("should detect git push", () => {
      expect(detectGitViolation("git push origin main")).toBe(ViolationType.DIRECT_GIT_COMMAND);
    });

    it("should detect git rebase", () => {
      expect(detectGitViolation("git rebase main")).toBe(ViolationType.DIRECT_GIT_COMMAND);
    });

    it("should detect git cherry-pick", () => {
      expect(detectGitViolation("git cherry-pick abc123")).toBe(ViolationType.DIRECT_GIT_COMMAND);
    });

    it("should detect git reset --hard", () => {
      expect(detectGitViolation("git reset --hard HEAD~1")).toBe(ViolationType.DIRECT_GIT_COMMAND);
    });
  ...
  describe("detectGhViolation", () => {
    it("should detect gh pr merge", () => {
      expect(detectGhViolation("gh pr merge 42")).toBe(ViolationType.DIRECT_MERGE);
    });

    it("should detect gh pr close", () => {
      expect(detectGhViolation("gh pr close 42")).toBe(ViolationType.DIRECT_GH_COMMAND);
    });
  ...
  describe("detectCiConfigViolation", () => {
    it("should detect .github/workflows modifications", () => {
      expect(detectCiConfigViolation(".github/workflows/ci.yml")).toBe(
        ViolationType.MODIFY_CI_CONFIG
      );
    });

    it("should detect .gitlab-ci.yml modifications", () => {
      expect(detectCiConfigViolation(".gitlab-ci.yml")).toBe(ViolationType.MODIFY_CI_CONFIG);
    });

    it("should detect Jenkinsfile modifications", () => {
      expect(detectCiConfigViolation("Jenkinsfile")).toBe(ViolationType.MODIFY_CI_CONFIG);
    });

    it("should detect .circleci modifications", () => {
      expect(detectCiConfigViolation(".circleci/config.yml")).toBe(ViolationType.MODIFY_CI_CONFIG);
    });

    it("should detect azure-pipelines.yml modifications", () => {
      expect(detectCiConfigViolation("azure-pipelines.yml")).toBe(ViolationType.MODIFY_CI_CONFIG);
    });
  ...
  describe("getViolationSeverity", () => {
    ...
    it("should return warning for DIRECT_GH_COMMAND", () => {
      expect(getViolationSeverity(ViolationType.DIRECT_GH_COMMAND)).toBe(ViolationSeverity.WARNING);
    });

    it("should return error for DIRECT_MERGE", () => {
      expect(getViolationSeverity(ViolationType.DIRECT_MERGE)).toBe(ViolationSeverity.ERROR);
    });

    it("should return error for BYPASS_GATES", () => {
      expect(getViolationSeverity(ViolationType.BYPASS_GATES)).toBe(ViolationSeverity.ERROR);
    });

    it("should return critical for MODIFY_CI_CONFIG", () => {
      expect(getViolationSeverity(ViolationType.MODIFY_CI_CONFIG)).toBe(ViolationSeverity.CRITICAL);
    });

    it("should return critical for FORBIDDEN_ACTION", () => {
      expect(getViolationSeverity(ViolationType.FORBIDDEN_ACTION)).toBe(ViolationSeverity.CRITICAL);
    });

    it("should return warning for SKIP_POLICY", () => {
      expect(getViolationSeverity(ViolationType.SKIP_POLICY)).toBe(ViolationSeverity.WARNING);
    });
  });
  ...
  describe("logViolation", () => {
    ...
    it("should persist violation to failures.ndjson", async () => {
      ...
      logViolation(run.runId, ViolationType.DIRECT_MERGE, { command: "git merge main" }, testDir);
  ...
  describe("countViolationsBySeverity", () => {
    it("should count violations by severity", async () => {
      ...
      logViolation(run.runId, ViolationType.DIRECT_GIT_COMMAND, {}, testDir); // warning
      logViolation(run.runId, ViolationType.DIRECT_GH_COMMAND, {}, testDir); // warning
      logViolation(run.runId, ViolationType.DIRECT_MERGE, {}, testDir); // error
      logViolation(run.runId, ViolationType.MODIFY_CI_CONFIG, {}, testDir); // critical
  ...
  describe("generateViolationRiskFlags", () => {
    ...
    it("should include total violation count", async () => {
      ...
      logViolation(run.runId, ViolationType.DIRECT_GIT_COMMAND, {}, testDir);
      logViolation(run.runId, ViolationType.DIRECT_GH_COMMAND, {}, testDir);
    ...
    it("should include critical count when present", async () => {
      ...
      logViolation(run.runId, ViolationType.MODIFY_CI_CONFIG, {}, testDir);
