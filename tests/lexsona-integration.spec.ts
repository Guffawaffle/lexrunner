...
describe("LexSona Integration", () => {
  ...
  describe("deriveShadowConstraints", () => {
    ...
    it("includes derivedAt timestamp", async () => {
      ...
      expect(new Date(result.derivedAt).getTime()).toBeLessThanOrEqual(Date.now());
    });
  });
  ...
  describe("createGovernanceComparisonLog", () => {
    it("creates valid log entry", () => {
      ...
      const log = createGovernanceComparisonLog(context, lexsona, runner, "shadow");
      ...
      expect(log.runner.gatesRequired).toEqual(["lint", "typecheck", "test"]);
  ...
  describe("formatShadowGovernanceSummary (QOL-003)", () => {
    it("formats successful result with agreement", () => {
      ...
      const summary = formatShadowGovernanceSummary(shadowResult, runnerSignals, { noColor: true });
