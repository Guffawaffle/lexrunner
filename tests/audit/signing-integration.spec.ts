...
describe("Audit Signing Integration", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "lex-audit-signing-int-")
		);
	});
	...
	it("should create audit manifest without signing when signer is not specified", async () => {
		...
		expect(fs.existsSync(path.join(tmpDir, "audit-manifest.sig"))).toBe(
			false
		);
		expect(
			fs.existsSync(path.join(tmpDir, "audit-manifest.sig.meta"))
		).toBe(false);
	});
	...
	it("should call signing when signer option is provided (KMS mocked)", async () => {
		...
		const keyArn =
			"arn:aws:kms:us-east-1:123456789012:key/integration-test";
	...
	it("should handle GPG signing failure gracefully", async () => {
		...
		await expect(finalizeAudit(emitter, "success")).rejects.toThrow(
			"GPG signing failed"
		);
	});
	...
	it("should propagate signing errors during finalize", async () => {
		...
		const mockSend = vi
			.fn()
			.mockRejectedValue(new Error("KMS AccessDeniedException"));
	...
	it("should work with encrypted audit logs and signing", async () => {
		...
		const encFile = manifest.files.find(
			(f: any) => f.file === "audit.ndjson.enc"
		);
	...
	it("should create deterministic manifest with signing metadata", async () => {
		...
		const keyArn =
			"arn:aws:kms:us-east-1:123456789012:key/deterministic-test";
		...
		for (let i = 1; i < files.length; i++) {
			expect(
				files[i].file.localeCompare(files[i - 1].file)
