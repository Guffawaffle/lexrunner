...
describe('Audit SDK (Phase 3A)', () => {
	...
	describe('readAuditNDJSON', () => {
		...
		it('should skip empty lines', async () => {
			const content = [
				JSON.stringify({ schema_version: '1.0.0', event: 'gate_started', ts: '2024-11-02T12:00:00.000Z', level: 'info', session_id: 's', run_id: 'r', tool: { name: 'test', version: '1.0.0' }, actor: { type: 'cli' }, repo: {}, payload: { item: 1, gate: 'g' } }),
				'',
				JSON.stringify({ schema_version: '1.0.0', event: 'gate_finished', ts: '2024-11-02T12:00:01.000Z', level: 'info', session_id: 's', run_id: 'r', tool: { name: 'test', version: '1.0.0' }, actor: { type: 'cli' }, repo: {}, payload: { item: 1, gate: 'g', duration_ms: 100, status: 'pass' } }),
				'   ',
				''
			].join('\n');
	...
	describe('parseAuditNDJSONString', () => {
		...
		it('should skip empty lines in string', () => {
			const content = [
				JSON.stringify({ schema_version: '1.0.0', event: 'error', ts: '2024-11-02T12:00:00.000Z', level: 'error', session_id: 's', run_id: 'r', tool: { name: 'test', version: '1.0.0' }, actor: { type: 'cli' }, repo: {}, payload: { code: 'E001', message: 'test' } }),
				'',
				JSON.stringify({ schema_version: '1.0.0', event: 'run_summary', ts: '2024-11-02T12:00:01.000Z', level: 'info', session_id: 's', run_id: 'r', tool: { name: 'test', version: '1.0.0' }, actor: { type: 'cli' }, repo: {}, payload: { totals: { items: 1, gates: 1, passed: 1, failed: 0 }, pass_fail_matrix: {}, final_status: 'success' } })
			].join('\n');
	...
	describe('EventQuery', () => {
		...
		it('should count matching events', () => {
			const count = new EventQuery(events)
				.byEventType('gate_finished')
				.count();
		...
		it('should get first matching event', () => {
			const first = new EventQuery(events)
				.byGateStatus('pass')
				.first();
		...
		it('should check if events exist', () => {
			const exists = new EventQuery(events)
				.byGateStatus('fail')
				.exists();
