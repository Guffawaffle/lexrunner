...
describe("paths: isSafeArtifactPath", () => {
  describe("should block PR artifact paths", () => {
    ...
    it("should block Windows-style paths with PR directories", () => {
      ...
      expect(() => isSafeArtifactPath("C:\\artifacts\\PR-456\\output.json")).toThrow(
        "SAFETY VIOLATION"
      );
    });
    ...
    it("should block /artifacts/PR-*/ paths", () => {
      ...
      expect(() => isSafeArtifactPath("/home/user/artifacts/pr-456/file.txt")).toThrow(
        "SAFETY VIOLATION"
      );
    });
  ...
  describe("should allow session deliverable paths", () => {
    it("should allow .smartergpt.local/deliverables/_session/", () => {
      ...
      expect(
        isSafeArtifactPath("/home/user/.smartergpt.local/deliverables/_session/spec.json")
      ).toBe(true);
    });
    ...
    it("should allow .smartergpt/deliverables/_session/", () => {
      ...
      expect(isSafeArtifactPath("/workspace/.smartergpt/deliverables/_session/output.txt")).toBe(
        true
      );
    });
...
describe("paths: validateOutputPath", () => {
  ...
  it("should throw for PR artifact paths", async () => {
    await expect(validateOutputPath("/path/to/pr-123/output.json")).rejects.toThrow(
      "SAFETY VIOLATION"
    );
  });

  it("should throw for artifacts/PR-* paths", async () => {
    await expect(validateOutputPath("artifacts/PR-456/spec.json")).rejects.toThrow(
      "SAFETY VIOLATION"
    );
  });
});
