...
describe('E2E: Synthetic 6-PR Weave with Conflicts', () => {
	...
	it('should complete 6-PR weave with 2 conflicts and green gates', async (ctx) => {
		...
		const { stdout: orderOutput } = await runCLI(
			`merge-order ${planPath} --json`
		);
	...
	it('should track budget and stay within limits', async (ctx) => {
		...
		// Assertions
		expect(spend.prompts).toBeLessThanOrEqual(
			scenario.expectedBudget.maxPrompts
		);
		expect(spend.tokens_estimated).toBeLessThanOrEqual(
			scenario.expectedBudget.maxTokens
		);
		// Verify not exceeded
