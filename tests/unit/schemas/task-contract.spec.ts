...
function createGoldenSnapshot(): TaskSnapshot_v1 {
	const snapshotWithoutHash = {
		...
		source_of_truth: {
			...
			excerpt:
				"export const INTERVENTION_CATALOG = { /* 19 interventions */ }",
			introducing_change: {
				commit_sha: "81102f1452a3ad0428879c32a02ede2771df63fc",
...
function createGoldenReceipt(snapshotHash: string): TaskReceipt_v1 {
	return {
		...
		claims: {
			...
			rationale:
				"Updated assertion to match new tool count after validate_remember was added",
			confidence: "high",
...
describe("Canonical Hash Helper", () => {
	describe("computeCanonicalHash", () => {
		...
		it("handles arrays (preserves order)", () => {
			const arr1 = [1, 2, 3];
			const arr2 = [3, 2, 1];
			expect(computeCanonicalHash(arr1)).not.toBe(
				computeCanonicalHash(arr2)
			);
		});
	});
	...
	describe("verifySnapshotBinding", () => {
		it("returns true when hashes match", () => {
			...
			expect(
				verifySnapshotBinding(snapshot, snapshot.snapshot_hash)
			).toBe(true);
		});
...
describe("EngineVerification_v1 Schema", () => {
	it("validates golden fixture", () => {
		const snapshot = createGoldenSnapshot();
		const receipt = createGoldenReceipt(snapshot.snapshot_hash);
		const verification = createGoldenVerification(
...
describe("Receipt Binding Validation", () => {
	...
	it("rejects floating receipts (snapshot_hash mismatch)", () => {
		const snapshot = createGoldenSnapshot();
		const receipt = createGoldenReceipt(
			"sha256:wrong_hash_prevents_floating_receipts"
		);
		const result = validateReceiptBinding(snapshot, receipt);
		expect(result.valid).toBe(false);
...
describe("Verification Binding Validation", () => {
	it("validates complete trio", () => {
		...
		const result = validateVerificationBinding(
			snapshot,
			receipt,
			verification
		);
	...
	it("rejects snapshot_hash mismatch in verification", () => {
		...
		expect(result.errors).toContainEqual(
			expect.stringContaining("Snapshot hash mismatch")
		);
	});
	...
	it("detects trust gap incorrectly computed", () => {
		...
		expect(result.valid).toBe(false);
		expect(result.errors).toContainEqual(
			expect.stringContaining("Trust gap incorrectly computed")
		);
	});
});
...
describe("RepoRelativePath Validation", () => {
	it("accepts valid repo-relative paths", () => {
		...
		expect(
			RepoRelativePath.safeParse("tests/unit/test.spec.ts").success
		).toBe(true);
		expect(RepoRelativePath.safeParse("file.ts").success).toBe(true);
	});

	it("rejects absolute paths (leading slash)", () => {
		expect(RepoRelativePath.safeParse("/absolute/path.ts").success).toBe(
			false
		);
	});

	it("rejects Windows absolute paths", () => {
		expect(RepoRelativePath.safeParse("C:\\path\\file.ts").success).toBe(
			false
		);
		expect(RepoRelativePath.safeParse("D:/path/file.ts").success).toBe(
			false
		);
	});

	it("rejects paths escaping repo root", () => {
		expect(RepoRelativePath.safeParse("../outside/file.ts").success).toBe(
			false
		);
	});
});
