...
describe("Test Fix Patterns Integration", () => {
  ...
  it("contains tool-count-assertion pattern", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      const pattern = result.patterns.patterns.find((p) => p.id === "tool-count-assertion");
  ...
  it("contains env-dependent-homedir pattern", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      const pattern = result.patterns.patterns.find((p) => p.id === "env-dependent-homedir");
      expect(pattern).toBeDefined();
  ...
  it("returns enabled patterns sorted by priority", () => {
    ...
    if (result.success) {
      ...
      for (let i = 1; i < enabled.length; i++) {
        expect(enabled[i - 1].priority).toBeGreaterThanOrEqual(enabled[i].priority);
      }
    }
  });
  ...
  it("all patterns have valid fix actions", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      const validActions = ["update_number", "update_string", "replace", "make_environment_aware"];
  ...
  it("all patterns have either test_output or error_message trigger", () => {
    ...
    if (result.success) {
      for (const pattern of result.patterns.patterns) {
        const hasTrigger = pattern.trigger.test_output || pattern.trigger.error_message;
