...
import { EngineVerifier, SnapshotMismatchError } from "../../../src/verification/index.js";
import type { TaskSnapshot_v1, TaskReceipt_v1 } from "../../../src/schemas/task-contract.js";
import { computeSnapshotHash, TASK_CONTRACT_VERSION } from "../../../src/schemas/task-contract.js";
...
describe("EngineVerifier", () => {
  ...
  describe("verify", () => {
    it("succeeds when verification passes", async () => {
      ...
      const receipt = createTestReceipt("test-001", snapshot.snapshot_hash, true);
    ...
    it("detects trust gap (agent claimed fixed but verification fails)", async () => {
      const snapshot = createTestSnapshot("test-002", 'echo "Tests failed" && exit 1', {
        exit_code: 0,
      });
...
`;
      ...
      const receipt = createTestReceipt("test-004", snapshot.snapshot_hash, true, unifiedDiff);
  ...
  describe("checkExpectations", () => {
    it("validates exit code", async () => {
      ...
      const receipt = createTestReceipt("test-005", snapshot.snapshot_hash, true);
    ...
    it("validates must_include patterns", async () => {
      const snapshot = createTestSnapshot("test-006", 'echo "Success: all tests passed"', {
        exit_code: 0,
        must_include: ["Success", "all tests"],
      });

      const receipt = createTestReceipt("test-006", snapshot.snapshot_hash, true);
    ...
    it("validates must_not_include patterns", async () => {
      const snapshot = createTestSnapshot("test-007", 'echo "All tests passed"', {
        exit_code: 0,
        must_not_include: ["FAILED", "Error"],
      });

      const receipt = createTestReceipt("test-007", snapshot.snapshot_hash, true);
    ...
    it("fails when must_not_include pattern is found", async () => {
      ...
      const receipt = createTestReceipt("test-008", snapshot.snapshot_hash, true);
