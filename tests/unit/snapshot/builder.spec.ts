...
describe("SnapshotBuilder", () => {
  ...
  beforeEach(async () => {
    ...
    // Copy fixture files
    await fs.copyFile(path.join(fixturesDir, "sample.ts"), path.join(tempDir, "sample.ts"));
    await fs.copyFile(path.join(fixturesDir, "package.json"), path.join(tempDir, "package.json"));
  });
  ...
  it("extractHunks handles file start/end boundaries", async () => {
    ...
    const lastNonEmptyLine = [...fileLines].reverse().find((l) => l.length > 0);
  ...
  it("hunk hash is deterministic", async () => {
    ...
    expect(snapshot1.targets[0].hunk_sha256).toBe(snapshot2.targets[0].hunk_sha256);
  ...
  it("includes failure evidence", async () => {
    ...
    expect(snapshot.failure.message).toBe("TypeError: Cannot divide by zero");
    expect(snapshot.failure.file_rel).toBe("sample.ts");
    expect(snapshot.failure.line).toBe(14);
