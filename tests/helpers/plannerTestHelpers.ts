...
export function assertPlanStructure(
	...
	if (expected.layers) {
		...
		if (plan.layers.length !== expected.layers.length) {
			throw new Error(
				`Expected ${expected.layers.length} layers, got ${plan.layers.length}`
			);
		}
	...
	if (expected.warnings !== undefined) {
		...
		if (actualWarnings !== expected.warnings) {
			throw new Error(
				`Expected ${expected.warnings} warnings, got ${actualWarnings}`
			);
		}
	}
	...
	if (expected.errors) {
		...
		if (plan.errors.length !== expected.errors.length) {
			throw new Error(
				`Expected ${expected.errors.length} errors, got ${plan.errors.length}`
			);
		}
	}
}
