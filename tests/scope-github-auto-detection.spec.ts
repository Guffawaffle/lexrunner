...
`;
      fs.writeFileSync(path.join(tempDir, ".smartergpt", "scope.yml"), scopeContent);
      fs.writeFileSync(path.join(tempDir, ".smartergpt", "stack.yml"), stackContent);
    ...
    it("should handle invalid scope.yml gracefully", () => {
      ...
      fs.writeFileSync(path.join(tempDir, ".smartergpt", "scope.yml"), invalidContent);
...
`;
      ...
      expect(result.shouldUseGitHub).toBe(true);
      expect(result.scopeConfig?.labels).toEqual(["stack:*", "priority:high", "team:backend"]);
    });
