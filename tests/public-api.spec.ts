...
describe("Public API - Frames Module", () => {
  it("should export frame types", async () => {
    const { ExecutionFrameSchema, validateExecutionFrame } =
      await import("../../src/frames/types.js");
  ...
  it("should export emitter functions", async () => {
    const { emitMergeWeaveFrame, emitExecutorFrame, emitGateFrame, emitProcedureFrame } =
  ...
  it("should export storage functions", async () => {
    const { storeFrame, storeFrameResult, readFrame, listFrameIds, listFrames, deleteFrame } =
...
describe("Public API - Errors Module", () => {
  it("should export error types and functions", async () => {
    const { createAXError, AXErrorException } = await import("../../src/errors/index.js");
  ...
  it("should create AXError correctly", async () => {
    ...
    const error = createAXError("TEST_ERROR", "Test error message", ["Action 1", "Action 2"]);
...
describe("Public API - Integration Test", () => {
  ...
  it("should handle errors using public API", async () => {
    ...
    const error = createAXError("PUBLIC_API_TEST", "Test error for public API validation", [
      "Debug the error",
      "Retry the operation",
    ]);
...
describe("Public API - Executors FrameContract Module", () => {
  ...
  it("should enforce frame emission requirement", async () => {
    const { enforceFrameEmission, FrameContractViolationError } =
      await import("../../src/executors/frameContract.js");
  ...
  it("should build frame from template", async () => {
    const { buildFrameFromTemplate } = await import("../../src/executors/frameContract.js");
