...
import type { GuardrailProfile, G_scope, AccessPattern } from "../../src/types/guardrails.js";
...
describe("guardrailEnforcement", () => {
  ...
  describe("checkFileAccess", () => {
    ...
    it("allows multiple allowed patterns", () => {
      ...
      expect(checkFileAccess("docs/README.md", "read", scope).allowed).toBe(false);
    });
  });
  ...
  describe("checkNetworkAccess", () => {
    ...
    it("allows access matching allow pattern", () => {
      ...
      expect(checkNetworkAccess("registry.npmjs.org", scope).allowed).toBe(true);
    });
