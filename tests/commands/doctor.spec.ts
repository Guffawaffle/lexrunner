...
describe("Doctor Command", () => {
	...
	it("should register doctor command with correct configuration", () => {
		...
		const environmentQualityOption = opts?.find(
			(opt) => opt.long === "--environment-quality"
		);
		expect(environmentQualityOption).toBeDefined();
		expect(environmentQualityOption?.description).toBe(
			"Run environmental hostility scoring"
		);
	});
