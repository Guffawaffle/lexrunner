...
describe("Schema Command Module", () => {
	...
	describe("registerSchemaCommand", () => {
		it("should register schema command with validate subcommand", () => {
			...
			expect(schemaCommand?.description()).toBe(
				"Schema utilities (validate plan.json)"
			);

			// Check that validate subcommand exists
			const validateCommand = schemaCommand?.commands.find(
				(cmd) => cmd.name() === "validate"
			);
			expect(validateCommand).toBeDefined();
			expect(validateCommand?.description()).toBe(
				"Validate a plan file against schema"
			);
		});
	...
	describe("schema validate command (CLI integration)", () => {
		it("should validate a valid plan file", (ctx) => {
			...
			fs.writeFileSync(planPath, canonicalJSONStringify(validPlan));

			const output = execSync(
				`node ${cliPath} schema validate ${planPath} --json`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should detect invalid plan file", (ctx) => {
			...
			let output: string;
			try {
				execSync(`node ${cliPath} schema validate ${planPath} --json`, {
					encoding: "utf8",
					stdio: "pipe",
				});
		...
		it("should handle missing file gracefully", (ctx) => {
			...
			try {
				execSync(
					`node ${cliPath} schema validate nonexistent.json --json`,
					{
						encoding: "utf8",
						stdio: "pipe",
					}
			...
			expect(result.errors).toEqual([
				{ path: "root", message: "File not found" },
			]);
		});
	});
});
