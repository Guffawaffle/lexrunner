...
describe("Retry Command", () => {
  ...
  describe("command execution", () => {
    ...
    it("should list items in dry-run mode", async () => {
      ...
      expect(consoleLogSpy.mock.calls.some((call: any) => call[0].includes("item-1:lint"))).toBe(
        true
      );
      expect(consoleLogSpy.mock.calls.some((call: any) => call[0].includes("item-2:test"))).toBe(
        true
      );
    });
  ...
  describe("error handling", () => {
    it("should handle failed items gracefully", async () => {
      ...
      await expect(program.parseAsync(["node", "test", "retry"])).rejects.toThrow();
    ...
    it("should call exitWith on unexpected errors", async () => {
      ...
      await expect(program.parseAsync(["node", "test", "retry"])).rejects.toThrow(testError);
