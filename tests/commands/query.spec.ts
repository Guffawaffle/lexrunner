...
describe("Query Command Tests", () => {
	...
	beforeEach((context) => {
		...
		// Create sample plan.json (use absolute path)
		fs.writeFileSync(
			path.join(testDir, "plan.json"),
			canonicalJSONStringify(samplePlan)
		);
	...
	describe("query with --stats", () => {
		it("should show plan statistics in table format", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --stats`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should output JSON with --format json", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --stats --format json`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("query with --roots", () => {
		it("should show root nodes", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --roots`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should output JSON with --format json", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --roots --format json`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("query with --leaves", () => {
		it("should show leaf nodes", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --leaves`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should output JSON with --format json", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --leaves --format json`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("query with --level", () => {
		it("should filter by level", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --level 1`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should output JSON with --format json", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --level 2 --format json`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("query with query string", () => {
		it("should filter by name contains", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} "name contains feature"`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should filter by gate count", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} "gatesCount eq 0" --format json`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should support AND operator", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} "level eq 2 AND gatesCount eq 1" --format json`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("query with --output", () => {
		it("should write results to file", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query ${planPath} --stats --output ${outputFile}`,
				{
					encoding: "utf8",
				}
			);
		...
		it("should write JSON to file", (ctx) => {
			...
			execSync(
				`node ${cliPath} query ${planPath} --roots --format json --output ${outputFile}`,
				{
					encoding: "utf8",
				}
			);
	...
	describe("error handling", () => {
		...
		it("should error when no query or option provided", (ctx) => {
			...
			expect(() => {
				execSync(`node ${cliPath} query ${planPath}`, {
					encoding: "utf8",
					stdio: "pipe",
				});
			}).toThrow();
	...
	describe("--plan option", () => {
		it("should accept plan via --plan option", (ctx) => {
			...
			const output = execSync(
				`node ${cliPath} query --plan ${planPath} --stats`,
				{
					encoding: "utf8",
				}
			);
