...
describe("validation: validateOrThrow", () => {
  ...
  it("should throw for invalid data", () => {
    ...
    expect(() => validateOrThrow(data, TestSchema, "Test Schema")).toThrow(
      "Schema validation failed for Test Schema"
    );
  });
  ...
  it("should log detailed error messages", () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("version"));
    }
  });
  ...
  it("should show type mismatch details", () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("Expected: string"));
      // Zod v4 may have different error message format, check for the type mismatch indication
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringMatching(/number|Invalid input/));
    }
  });
...
describe("validation: preflightSchemaCheck", () => {
  ...
  it("should validate a valid spec file", async () => {
    ...
    await expect(preflightSchemaCheck(specPath, TestSchema, "Test Schema")).resolves.not.toThrow();

    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("Pre-flight schema check: Test Schema")
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Schema validation passed"));
  });
  ...
  it("should reject an invalid spec file", async () => {
    ...
    await expect(preflightSchemaCheck(specPath, TestSchema, "Test Schema")).rejects.toThrow(
      "Schema validation failed for Test Schema"
    );
  });
