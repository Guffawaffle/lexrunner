...
`
				...
				const output = execSync(`node ${cliPath} config validate --profile-dir ${testProfileDir} --json`, {
					encoding: 'utf8'
				});
...
`
				...
				try {
					...
					expect(result.errors.some((e: any) => e.message.includes('YAML syntax error'))).toBe(true);
				}
			});
		});
...
`
				...
				expect(result.warnings.some((w: any) => w.message.includes("Referenced gate 'e2e'"))).toBe(true);
			});
		});
...
`
				...
				try {
					execSync(`node ${cliPath} config validate --profile-dir ${testProfileDir} --strict --json`, {
						encoding: 'utf8',
						stdio: 'pipe'
					});
					throw new Error('Should have failed');
