...
describe("config validate command", () => {
	const testDir = path.join(
		os.tmpdir(),
		"lex-pr-runner-config-validate-test"
	);
	...
	describe("command registration", () => {
		it("should register config validate command", () => {
			...
			const validateCommand = configCommand?.commands.find(
				...
				try {
					...
					expect(
						result.errors.some((e: any) =>
							e.message.includes("YAML syntax error")
						)
					).toBe(true);
				}
			});
...
`
				...
				expect(
					result.warnings.some((w: any) =>
						w.message.includes("Referenced gate 'e2e'")
					)
				).toBe(true);
			});
		});
