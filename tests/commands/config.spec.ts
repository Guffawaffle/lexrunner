...
describe("Config Command", () => {
	...
	it("should register config command with show subcommand", () => {
		...
		const showCommand = configCommand?.commands.find(
			(cmd) => cmd.name() === "show"
		);
		expect(showCommand).toBeDefined();
		expect(showCommand?.description()).toBe(
			"Display configuration with precedence chain"
		);
	});
	...
	it("should register with program successfully", () => {
		...
		expect(program.commands.some((cmd) => cmd.name() === "config")).toBe(
			true
		);
	});
});
...
describe("config show command (CLI integration)", () => {
	...
	beforeEach(() => {
		...
		fs.writeFileSync(
			path.join(smartergptDir, "scope.yml"),
			"version: 1\ntarget: main\n"
		);
	});
	...
	it("should filter by specific key", (ctx) => {
		...
		const output = execSync(
			`node ${cliPath} config show --key scope.target`,
			{
				encoding: "utf-8",
				cwd: testDir,
			}
	...
	it("should output JSON for specific key with --json", (ctx) => {
		...
		const output = execSync(
			`node ${cliPath} config show --key scope.target --json`,
			{
				encoding: "utf-8",
				cwd: testDir,
			}
	...
	it("should display multiple config files", (ctx) => {
		...
		expect(keys.some((k: string) => k.startsWith("gates.levels"))).toBe(
			true
		);
	});
	...
	it("should handle non-existent key gracefully", (ctx) => {
		...
		try {
			...
			expect.fail(
				"Expected command to exit with error for non-existent key"
			);
		} catch (error: any) {
	...
	it("should handle environment variable LEX_PR_PROFILE_DIR", (ctx) => {
		...
		fs.writeFileSync(
			path.join(customProfileDir, "scope.yml"),
			"version: 1\ntarget: production\n"
		);
...
`
		...
		const result = JSON.parse(output);
		const gatesConfig = result.configuration.find(
			(c: any) => c.key === "gates.levels.default"
		);
