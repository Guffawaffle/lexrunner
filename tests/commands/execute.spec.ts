...
describe('Execute Command', () => {
	...
	describe('command registration', () => {
		it('should register execute command with correct name', () => {
			...
			expect(executeCommand?.description()).toBe('Execute plan with policy-aware gate running and status tracking');
		});
		...
		it('should have correct default values', () => {
			...
			const artifactDirOption = executeCommand?.options.find(opt => opt.long === '--artifact-dir');
			...
			const branchPrefixOption = executeCommand?.options.find(opt => opt.long === '--branch-prefix');
			...
			const auditFormatOption = executeCommand?.options.find(opt => opt.long === '--audit-format');
			expect(auditFormatOption?.defaultValue).toBe('jsonl');

			const auditSampleOption = executeCommand?.options.find(opt => opt.long === '--audit-sample');
