...
describe('Autopilot Command', () => {
	...
	describe('command registration', () => {
		it('should register autopilot command with correct name', () => {
			...
			expect(autopilotCommand?.description()).toBe('Run autopilot analysis and artifact generation');
		});
		...
		it('should have correct option descriptions', () => {
			...
			const deliverablesOption = autopilotCommand?.options.find(opt => opt.long === '--deliverables-dir');
			expect(deliverablesOption?.description).toBe('Custom deliverables directory (overrides default profile/deliverables)');
	...
	describe('error handling', () => {
		it('should error when no plan file is provided', async () => {
			await expect(
				program.parseAsync(['node', 'test', 'autopilot'])
			).rejects.toThrow();
