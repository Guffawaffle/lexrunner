...
import { getDb, closeDb, saveFrame, searchFrames } from "@smartergpt/lex/store";
...
describe("Lex Integration Smoke Test", () => {
  ...
  describe("Frame Recall (searchFrames)", () => {
    ...
    it("should recall frames by module scope", () => {
      ...
      expect(results.frames.length).toBeGreaterThan(0);
      const authFrame = results.frames.find((f) => f.module_scope.includes("services/auth"));
  ...
  describe("Module ID Validation (validateModuleIds)", () => {
    const testPolicy: Policy = {
      critical_rule: "Modules must be explicitly defined in policy before use",
    ...
    it("should validate existing module IDs", async () => {
      const result = await validateModuleIds(["services/auth", "services/user"], testPolicy);
    ...
    it("should detect invalid module IDs", async () => {
      const result = await validateModuleIds(["services/auth", "nonexistent/module"], testPolicy);
  ...
  describe("Atlas Frame Generation (generateAtlasFrame)", () => {
    ...
    it("should include seed module and its neighbors", () => {
      const atlasFrame = generateAtlasFrame(["services/auth"], 1, tempPolicyPath);

      // Should include the seed module
      const authModule = atlasFrame.modules.find((m) => m.id === "services/auth");
    ...
    it("should expand with larger fold radius", () => {
      const radius1 = generateAtlasFrame(["services/auth"], 1, tempPolicyPath);
      const radius2 = generateAtlasFrame(["services/auth"], 2, tempPolicyPath);

      // Larger radius should include at least as many modules
      expect(radius2.modules.length).toBeGreaterThanOrEqual(radius1.modules.length);
    });
  });
