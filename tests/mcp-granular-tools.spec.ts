...
describe('MCP Granular Plan Tools', () => {
	...
	describe('PlanValidateArgs schema validation', () => {
		it('should validate valid plan_validate arguments', () => {
			...
			expect(() => PlanValidateArgs.parse({ planContent: '{"schemaVersion":"1.0.0"}' })).not.toThrow();
			expect(() => PlanValidateArgs.parse({ planFile: '/path/to/plan.json', planContent: '{}' })).not.toThrow();
		});
