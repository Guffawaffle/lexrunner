...
import { createProcedureLoader, ProcedureStateMachine } from "../../src/procedures/index.js";
...
describe("PR Review Procedure", () => {
  ...
  describe("State Machine Traversal - Loopback", () => {
    it("transitions from decision_findings back to preparing on NEEDS_MORE_CONTEXT", () => {
      ...
      expect(procedure.applyTransition("decision_findings", "NEEDS_MORE_CONTEXT")).toBe(
        "preparing"
      );
    });
  ...
  describe("Next Options", () => {
    ...
    it("returns LLM decision options for decision_findings state", () => {
      ...
      expect(options.map((o) => o.action).sort()).toEqual([
        "abort",
        "approve",
        "needs_more_context",
        "request_changes",
