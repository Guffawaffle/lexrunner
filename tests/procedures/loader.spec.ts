...
describe("ProcedureLoader", () => {
  ...
  const writeProcedure = (filename: string, content: object): void => {
    fs.writeFileSync(path.join(tempDir, filename), YAML.stringify(content), "utf8");
  };
  ...
  it("throws for non-existent procedure", async () => {
    await expect(loader.loadProcedure("nonexistent")).rejects.toThrow(ProcedureLoadError);
  });
  ...
  it("throws for ID mismatch", async () => {
    ...
    await expect(loader.loadProcedure("mismatch")).rejects.toThrow(/ID mismatch/);
  });

  it("throws for invalid YAML", async () => {
    fs.writeFileSync(path.join(tempDir, "invalid.yaml"), "{ unclosed: brace", "utf8");

    await expect(loader.loadProcedure("invalid")).rejects.toThrow(ProcedureLoadError);
  });
  ...
  it("throws for invalid schema", async () => {
    ...
    await expect(loader.loadProcedure("bad-schema")).rejects.toThrow(ProcedureLoadError);
  });
