...
describe("InMemoryRunStore", () => {
	...
	describe("constructor", () => {
		...
		it("should pre-populate with runs", async () => {
			const runs = [
				createRunRecord({ runId: "run-1" }),
				createRunRecord({ runId: "run-2" }),
			];
		...
		it("should pre-populate with receipts", async () => {
			const receipts = [
				createReceipt({ receiptId: "receipt-1", runId: "run-1" }),
			];
	...
	describe("createRun", () => {
		...
		it("should throw error for duplicate runId", async () => {
			...
			await expect(store.createRun(run)).rejects.toThrow(
				'Run with runId "run-001" already exists'
			);
		});
	});
	...
	describe("updateRun", () => {
		...
		it("should throw error for unknown runId", async () => {
			await expect(
				store.updateRun("unknown-run", { state: "running" })
			).rejects.toThrow('Run with runId "unknown-run" not found');
		});
	});
