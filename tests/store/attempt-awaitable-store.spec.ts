...
import type { WorkspaceLifecycleStore } from "../../src/store/workspace-lifecycle-store.js";
      ...
      try {
        ...
        expect(() =>
          ExternalAwaitableDescriptor_v1.parse({
            ...descriptor(),
            subject: { repository: "owner/repo", token: "must-not-persist" },
          })
        ).toThrow(/must not contain credentials/u);
        expect(() => terminalResult("satisfied", T2, { access_token: "must-not-persist" })).toThrow(
          /evidence must not contain credentials/u
        );
...
function terminalResult(
  ...
  observedAt: string,
  evidence: unknown = { headSha: "a".repeat(40) }
) {
...
async function createLiveAttempt(store: Harness): Promise<void> {
