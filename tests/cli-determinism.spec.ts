...
describe("CLI Determinism Integration Tests", () => {
	...
	const testDir = path.join(
		os.tmpdir(),
		`lex-pr-runner-determinism-test-${path.basename(__filename)}`
	);
...
`
			...
			// Generate plan first
			const planOutput1 = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});
			const planOutput2 = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});

			expect(planOutput1).toBe(planOutput2);
			expect(sha256(Buffer.from(planOutput1))).toBe(
				sha256(Buffer.from(planOutput2))
			);
			...
			const mergeOutput1 = execSync(
				`node ${cliPath} merge-order plan.json --json`,
				{ encoding: "utf8" }
			);
			const mergeOutput2 = execSync(
				`node ${cliPath} merge-order plan.json --json`,
				{ encoding: "utf8" }
			);

			expect(mergeOutput1).toBe(mergeOutput2);
			expect(sha256(Buffer.from(mergeOutput1))).toBe(
				sha256(Buffer.from(mergeOutput2))
			);

			// Test status determinism
			const statusOutput1 = execSync(
				`node ${cliPath} status plan.json --json`,
				{ encoding: "utf8" }
			);
			const statusOutput2 = execSync(
				`node ${cliPath} status plan.json --json`,
				{ encoding: "utf8" }
			);

			expect(statusOutput1).toBe(statusOutput2);
			expect(sha256(Buffer.from(statusOutput1))).toBe(
				sha256(Buffer.from(statusOutput2))
			);

			// Test execute dry-run determinism
			const executeOutput1 = execSync(
				`node ${cliPath} execute plan.json --dry-run --json`,
				{ encoding: "utf8" }
			);
			const executeOutput2 = execSync(
				`node ${cliPath} execute plan.json --dry-run --json`,
				{ encoding: "utf8" }
			);

			expect(executeOutput1).toBe(executeOutput2);
			expect(sha256(Buffer.from(executeOutput1))).toBe(
				sha256(Buffer.from(executeOutput2))
			);
		});
	...
	describe("Environment state isolation", () => {
		it("should produce identical output regardless of working directory", (ctx) => {
			...
			for (const dir of [dir1, dir2]) {
				...
				fs.mkdirSync(path.join(dir, ".smartergpt"), {
					recursive: true,
				});
				fs.writeFileSync(
...
`
			...
			const output1 = execSync(`node ${cliPath} plan --json`, {
				cwd: dir1,
				encoding: "utf8",
			});

			const output2 = execSync(`node ${cliPath} plan --json`, {
				cwd: dir2,
				encoding: "utf8",
			});
...
`
			...
			const testEnvs = [
				{ ...baselineEnv, RANDOM_VAR: "value1" },
				{
					...baselineEnv,
					ANOTHER_VAR: "value2",
					PATH: baselineEnv.PATH + ":/fake/path",
				},
...
`
			...
			const singleItemOutput = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});
...
`
			...
			const twoItemOutput = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});
			...
			const singleItemOutput2 = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});
...
`
			...
			const planOutput = execSync(`node ${cliPath} plan --json`, {
				encoding: "utf8",
			});
...
`
	...
	describe("Error output determinism", () => {
		it("should produce consistent error JSON across multiple attempts", (ctx) => {
			...
			try {
				execSync(
					`node ${cliPath} schema validate invalid-plan.json --json`,
					{
						encoding: "utf8",
						stdio: "pipe",
					}
  ...
  "items": [
    {
      ...
      "deps": ["nonexistent-dependency"]
			...
			try {
				execSync(
					`node ${cliPath} merge-order nonexistent-ref.json --json`,
					{
						encoding: "utf8",
						stdio: "pipe",
					}
