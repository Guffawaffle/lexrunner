...
import { purgeCache, acquireRunnerLock, releaseRunnerLock, initializeFileLogger, closeFileLogger } from '../src/cli/runnerLifecycle';
...
describe('Runner Lifecycle Integration', () => {
	...
	beforeEach(() => {
		...
		fs.writeFileSync(
			path.join(testProfileDir, 'profile.yml'),
			'role: test\nname: test-profile\n'
		);
	});
