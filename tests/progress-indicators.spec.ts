...
describe("Progress Indicators", () => {
	describe("ProgressReporter", () => {
		...
		it("should show level start/complete in human mode", () => {
			...
			expect(logs.some((log) => log.includes("Level 1: Starting"))).toBe(
				true
			);
			expect(logs.some((log) => log.includes("[item-a, item-b]"))).toBe(
				true
			);
		});
		...
		it("should show node start in human mode", () => {
			...
			expect(logs.some((log) => log.includes("my-node: Starting"))).toBe(
				true
			);
		});
		...
		it("should show node completion tick for operations >2s", async () => {
			...
			expect(
				logs.some((log) => log.includes("slow-node: Starting"))
			).toBe(true);
			expect(
				logs.some((log) => log.includes("slow-node: Completed"))
			).toBe(true);
		});
		it("should NOT show node completion tick for operations <2s", async () => {
			...
			expect(
				logs.some((log) => log.includes("fast-node: Starting"))
			).toBe(true);
			expect(
				logs.some((log) => log.includes("fast-node: Completed"))
			).toBe(false);
		});
		...
		it("should format durations correctly", async () => {
			...
			const completionLog = logs.find((log) =>
				log.includes("node-seconds: Completed")
			);
		...
		it("should show success/failure icons correctly", async () => {
			...
			const successLog = logs.find((log) =>
				log.includes("success-node: Completed")
			);
			const failLog = logs.find((log) =>
				log.includes("fail-node: Completed")
			);
