...
export class TelemetryCollector {
  ...
  async flush(): Promise<void> {
    ...
    await Promise.all(this.config.stores.map((store) => store.write(events)));
  }
...
export function generateSessionReport(events: TelemetryEvent[], sessionId: string): SessionReport {
  ...
  const uncertainties = sessionEvents.filter(
    (e) => e.eventType === EventTypes.UNCERTAINTY_EXPRESSED
  );
  ...
  return {
    ...
    turns: {
      ...
      renegotiationRate: turnEnds.length > 0 ? renegotiations.length / turnEnds.length : 0,
    },
    ...
    .action(async (options) => {
      ...
      const refresh = async () => {
        ...
        for (const m of turnMetrics.slice(-5)) {
          console.log(
            `  ${m.sessionId.slice(0, 12)}  Turns: ${m.turnCount}  Renego: ${m.renegotiationTurns}`
          );
        }
        console.log();
...
export const ALERT_DEFINITIONS: AlertDefinition[] = [
  {
    ...
    condition: (m: TurnCostMetrics) => m.turnCount > 3 && m.renegotiationTurns / m.turnCount > 0.25,
    severity: "warning",
    message: (m) =>
  ...
  },
  {
    ...
    condition: (e: TelemetryEvent) => e.eventType === EventTypes.CONSTRAINT_VIOLATION,
    severity: "error",
  ...
  },
  {
    ...
    severity: "warning",
    message: (m) => `Escalation rate ${((m.escalations / m.tasks) * 100).toFixed(1)}% exceeds 30%`,
  },
];
```
...
_Next: [10-FAILURE-MODES.md](./10-FAILURE-MODES.md) — Known risks, biases, and limitations_
