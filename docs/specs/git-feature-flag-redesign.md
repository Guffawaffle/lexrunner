...
This document describes the centralized git wrapper implementation that provides safe defaults for automation and testing environments.
...
A centralized `runGit` wrapper in `src/shared/git/runGit.ts` that:

1. Disables GPG signing by default (`-c commit.gpgsign=false`)
2. Provides consistent return type: `{ exitCode, stdout, stderr }`
3. Supports dry-run mode via `LEX_GIT_MODE=off` environment variable
4. Offers helper functions for common operations
