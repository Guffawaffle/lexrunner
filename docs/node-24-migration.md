...
LexRunner 1.2.1 established the Ecosystem 3.1 Node.js 24 floor, and LexRunner 1.3.0 preserves it.
This intentional compatibility change applies to the `lex-pr` CLI, `lexrunner-mcp`, SDK imports,
local contributors, CI, and downstream consumers. “3.1” is the ecosystem sprint name, not the
LexRunner package major/minor.
...
Push-Location $smoke
npm init -y
npm install @smartergpt/lexrunner@1.3.0
npx lex-pr --help
...
CLI result, MCP `tools/list` result, and timestamp in the release evidence tracked by issue #881.
