...
function coerceBenchmarkMode(value: string | undefined): BenchmarkMode | undefined {
  if (!value) {
    return undefined;
  }
...
function generateSmokeMarkdownReport(results: BenchmarkResult[]): string {
  ...
  for (const r of sorted) {
    lines.push(`| ${r.suite} | ${r.name} | ${formatTime(r.meanTime)} | ${r.samples} |`);
  }

  lines.push("");
  lines.push("---");
  lines.push("To check regressions locally, run: `npm run benchmark:compare`.");
...
async function main() {
  const mode: BenchmarkMode =
    coerceBenchmarkMode(getArgValue("--mode")) ?? (process.env.CI ? "smoke" : "compare");
  ...
  // Run benchmarks
  try {
    execSync(`npm run benchmark -- --run --outputJson ${RAW_RESULTS_PATH}`, {
      stdio: "inherit",
    });
  } catch (error) {
  ...
  if (raw?.files && Array.isArray(raw.files)) {
    for (const file of raw.files) {
      for (const group of file?.groups || []) {
        const suite = normalizeSuiteName(group?.fullName || group?.name || "");
        for (const bench of group?.benchmarks || []) {
          ...
          results.push({
            ...
            suite: suite || "Unknown",
            meanTime: typeof bench?.mean === "number" ? bench.mean : 0,
