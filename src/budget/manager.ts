...
export class UnifiedBudgetExceededError extends Error {
	constructor(
		...
		super(
			`Budget exhausted: ${field} (${current}/${limit}) in ${scope} budget ${budgetId}`
		);
...
export class UnifiedBudgetManager {
	...
	checkBudget(budgetId: string, request: SpendRequest): void {
		...
		if (
			request.escalations !== undefined &&
			request.escalations > budget.escalations.remaining
		) {
			throw new UnifiedBudgetExceededError(
				"escalations",
	...
	spend(budgetId: string, request: SpendRequest): boolean {
		...
		if (request.escalations !== undefined) {
			budget.escalations.used += request.escalations;
			budget.escalations.remaining =
				budget.escalations.limit - budget.escalations.used;
		}
	...
	private propagateSpendToParent(parentId: string, request: SpendRequest): void {
		...
		if (request.tokens !== undefined) {
			...
			parent.tokens.remaining = Math.max(
				0,
				parent.tokens.limit - parent.tokens.used
			);
		}

		if (request.turns !== undefined) {
			parent.turns.used += request.turns;
			parent.turns.remaining = Math.max(
				0,
				parent.turns.limit - parent.turns.used
			);
		}

		if (request.time !== undefined) {
			parent.time.used += request.time;
			parent.time.remaining = Math.max(
				0,
				parent.time.limit - parent.time.used
			);
		}
...
export function createBudgetManager(
	options?: { receiptLogger?: (receipt: BudgetExhaustedReceipt) => void }
