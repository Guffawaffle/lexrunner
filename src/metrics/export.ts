...
export class GovernanceMetricsCollector {
  ...
  recordBudgetRemaining(summary: BudgetSummary): void {
    const tokensRemaining = Math.max(0, summary.tokenBudget - summary.tokens_estimated);
    const promptsRemaining = Math.max(0, summary.maxPrompts - summary.prompts);
    ...
    this.budgetRemainingMetrics = {
      ...
      tokenUtilization:
        summary.tokenBudget > 0 ? summary.tokens_estimated / summary.tokenBudget : 0,
      promptUtilization: summary.maxPrompts > 0 ? summary.prompts / summary.maxPrompts : 0,
    };
  }
  ...
  exportPrometheus(): string {
    ...
    if (this.turnCostMetrics !== null) {
      ...
      lines.push(`${METRIC_DEFINITIONS.lex_turn_cost_total.name} ${this.turnCostMetrics.total}`);
    }
 ...
 */
export function createMetricsCollector(sessionId?: string): GovernanceMetricsCollector {
