...
export async function createDraftPRForFailure(
  ...
  const { owner, repo, baseBranch, failureBranch, clusterIndex, failureBundle, artifactPaths } =
    options;
...
function generatePRBody(bundle: ClusterFailureBundle, artifactPaths: string[]): string {
  ...
  sections.push("");
  sections.push(
    `This is an automated draft PR created after gate failures during merge-weave execution.`
  );
  sections.push("");
  ...
  if (bundle.mergePatchDiff) {
    ...
    sections.push("");
    sections.push(
      `Full diff available in \`.weave/cluster-${bundle.clusterIndex}-merge-patch.diff\``
    );
    sections.push("");
  }
