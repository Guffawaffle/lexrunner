 ...
 */
const handleDiscoverPRs: InterventionHandler<"discover_prs"> = async (
	intervention,
	ctx
) => {
 ...
 */
const handleFilterDrafts: InterventionHandler<"filter_drafts"> = async (
	intervention,
	_ctx
) => {
	const start = Date.now();
 ...
 */
const handleParseDependencies: InterventionHandler<
	"parse_dependencies"
> = async (intervention, _ctx) => {
	const start = Date.now();
	const startedAt = new Date().toISOString();
 ...
 */
const handleRunBaseGate: InterventionHandler<"run_base_gate"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
 ...
 */
const handleCheckCIStatus: InterventionHandler<"check_ci_status"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
	...
	try {
		...
		const status = await ctx.github.getCommitStatus(
			owner,
			repo,
			pr.head.sha
		);
 ...
 */
const handleComputeMergeOrder: InterventionHandler<
	"compute_merge_order"
> = async (intervention, _ctx) => {
	const start = Date.now();
	const startedAt = new Date().toISOString();
 ...
 */
const handleCheckAdminAuthority: InterventionHandler<
	"check_admin_authority"
> = async (intervention, ctx) => {
	const start = Date.now();
	const startedAt = new Date().toISOString();
 ...
 * Merge a PR
 */
const handleExecuteMerge: InterventionHandler<"execute_merge"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
	...
	try {
		const { method, commitTitle, owner, repo, prNumber } =
			intervention.params as {
		...
		const success = await ctx.github.mergePullRequest(
			owner,
			repo,
			prNumber,
			{
 ...
 */
const handlePullChanges: InterventionHandler<"pull_changes"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
	...
	try {
		...
		const repoPath =
			ctx.repoPaths.get(`${owner}/${name}`) ?? ctx.workspaceRoot;
 ...
 */
const handleVerifyGates: InterventionHandler<"verify_gates"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
 ...
 */
const handleCommitFix: InterventionHandler<"commit_fix"> = async (
	intervention,
	ctx
) => {
	const start = Date.now();
...
export async function executeD1Intervention(
	...
	if (!handler) {
		throw new Error(
			`No D1 handler for intervention type: ${intervention.type}`
		);
	}
