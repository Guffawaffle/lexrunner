...
export async function executeClusterWithGates(
  ...
  try {
    ...
    if (!options?.skipGates) {
      ...
      if (!allGatesPassed) {
        console.error(`❌ Cluster ${clusterIndex} gates failed - initiating rollback`);

        // Store merge-patch.diff before rollback
        const mergePatchDiff = await captureMergePatchDiff(gitOps, preClusterSha);
        ...
        const bundlePath = await storeFailureBundle(weaveDir, clusterIndex, failureBundle);
 ...
 */
async function captureMergePatchDiff(gitOps: GitOperations, baseSha: string): Promise<string> {
  try {
 ...
 */
async function rollbackCluster(gitOps: GitOperations, targetSha: string): Promise<void> {
  try {
...
function createFailureBundle(
  ...
  const failedGates = gateResults
    ...
    .map((gr) => ({
      item: cluster.items.find((i) => i.gates?.some((g) => g.name === gr.gate))?.name || "unknown",
      gate: gr.gate,
      status: gr.status,
...
async function storeFailureBundle(
  ...
  const bundlePath = path.join(weaveDir, `cluster-${clusterIndex}-failure-bundle.json`);
...
function emitClusterFailureReceipt(
  ...
  const failedGateNames = bundle.failedGates.map((fg) => `${fg.item}:${fg.gate}`).join(", ");
...
async function createDraftPRForClusterFailure(
  ...
  try {
    ...
    const failureBranch = await createFailureBranch(cluster.clusterIndex, baseDir || process.cwd());
