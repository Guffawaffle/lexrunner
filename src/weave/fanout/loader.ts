 ...
 */
export async function loadFanoutTemplates(filePath: string): Promise<FanoutTemplates> {
  try {
    ...
    throw new FanoutTemplatesLoadError(
      `Failed to load fanout templates: ${error instanceof Error ? error.message : String(error)}`,
      filePath,
      error
    );
  }
 ...
 */
export async function loadFanoutTemplatesOrNull(filePath: string): Promise<FanoutTemplates | null> {
  try {
