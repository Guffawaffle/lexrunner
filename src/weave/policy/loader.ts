...
export async function loadPolicy(
	...
	try {
		...
		throw new PolicyLoadError(
			`Failed to parse YAML in policy file: ${resolvedPath}`,
			err as Error
		);
	}
	...
	try {
		...
		throw new PolicyLoadError(
			`Policy validation failed: ${resolvedPath}`,
	...
	if (!options.skipVersionCheck) {
		try {
			...
			throw new PolicyLoadError(
				`Policy version incompatible: ${resolvedPath}`,
				err as Error
			);
		}
	}
...
export async function loadPolicyOrNull(
	...
	try {
		...
		if (
			err instanceof PolicyLoadError &&
