			...
			if (
				...
				if (!condition.allow_pending || check.status === "completed") {
					return {
						type: "ci_status",
						passed: false,
						message: `Check ${checkName} not successful: ${
...
async function evaluateReviewCondition(
	...
	if (condition.no_pending_requests) {
		const pendingCount =
			context.reviewRequests.users.length +
			context.reviewRequests.teams.length;
		if (pendingCount > 0) {
			return {
				type: "review",
				passed: false,
	...
	if (condition.min_approvals > 0) {
		const approvals = context.reviews.filter(
			(r) => r.state === "APPROVED"
		).length;
	...
	if (condition.block_on_changes_requested) {
		const changesRequested = context.reviews.some(
...
async function evaluateApiCondition(
	...
	try {
		...
		const response = await deps.githubClient.request(
			condition.method,
			endpoint
		);
	...
	for (const trigger of triggers) {
		...
		if (trigger.label_present) {
			if (
				context.labels.some(
					(l) =>
						l.toLowerCase() === trigger.label_present?.toLowerCase()
				)
			) {
				return {
					required: true,
					reason:
						trigger.reason ??
						`Label ${trigger.label_present} requires escalation`,
				};
			}
		}
		...
		if (trigger.author_not_in) {
			if (
				...
				return {
					...
					reason:
						trigger.reason ??
						`Author ${context.author} requires escalation`,
				};
			}
		}
				...
				for (const file of context.files) {
					if (minimatch(file, pattern, { matchBase: true })) {
						return {
							...
							reason:
								trigger.reason ??
								`File ${file} matches escalation pattern`,
						};
					}
				}
			}
...
export async function evaluateAdminAuthority(
	...
	return {
		...
		escalation_reason: allPassed
			? undefined
			: `Conditions failed: ${conditionsFailed.join(", ")}`,
