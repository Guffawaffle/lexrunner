...
import type { InterventionPlan, PlannedIntervention } from "../planner/index.js";
...
export function createAuditLogger(options: AuditLoggerOptions) {
  ...
  return {
    ...
    async logSummary(
      ...
      const skipped = interventions.filter((i) => i.status === "skipped").length;

      const byDeterminism: Record<string, { completed: number; failed: number }> = {};
      for (const i of interventions) {
        if (!byDeterminism[i.determinism]) {
          byDeterminism[i.determinism] = { completed: 0, failed: 0 };
        }
        if (i.status === "completed") byDeterminism[i.determinism].completed++;
        if (i.status === "failed") byDeterminism[i.determinism].failed++;
      }
 ...
 */
export function createAuditEmitter(logger: AuditLogger): (event: AuditEvent) => void {
