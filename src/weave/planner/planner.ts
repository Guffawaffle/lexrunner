...
function toPlannedInterventions(
	...
	for (const intervention of interventions) {
		const status: PlannedIntervention["status"] = completedIds.has(
			intervention.id
		)
			? "completed"
...
export function createInterventionPlan(
	...
	const lastDiscoveryId =
		discoveryInterventions[discoveryInterventions.length - 1]?.id ?? "";
	...
	const lastMergeId =
		mergeInterventions[mergeInterventions.length - 1]?.id ?? "";

	const postMergeInterventions = planPostMergePhase(
		policy,
		policy.discovery.repos,
		lastMergeId
	);
 ...
 */
export function getReadyInterventions(
