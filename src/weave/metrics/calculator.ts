...
export function calculateInterventionStats(
	...
	const interventionEntries = entries.filter(
		(e) => e.intervention_id === interventionId
	);
	...
	const overrides = interventionEntries.filter(
		(e) => e.required_human_override
	).length;
	const totalTime = interventionEntries.reduce(
		(sum, e) => sum + e.time_to_complete_ms,
		0
	);
	...
	return {
		...
		determinism_level:
			catalog?.determinism_level ?? first.determinism_level,
		total_executions: interventionEntries.length,
		successful_executions: successful,
		success_rate:
			interventionEntries.length > 0
				? successful / interventionEntries.length
				: 0,
		human_overrides: overrides,
		avg_time_ms:
			interventionEntries.length > 0
				? totalTime / interventionEntries.length
				: 0,
	};
}
...
export function calculateLevelStats(
	...
	const uniqueInterventions = new Set(
		levelEntries.map((e) => e.intervention_id)
	);
	...
	return {
		...
		success_rate:
			levelEntries.length > 0 ? successful / levelEntries.length : 0,
...
export function generateHandoffReport(
	...
	if (d1Ready && !d1Ready.ready) {
		...
		if (d1Ready.success_rate < d1Ready.target_rate) {
			recommendations.push(
				`Improve D1 success rate from ${(
					d1Ready.success_rate * 100
				).toFixed(1)}% to ${(d1Ready.target_rate * 100).toFixed(1)}%`
			);
		}
	...
	if (d2Ready && !d2Ready.ready) {
		...
		if (d2Ready.success_rate < d2Ready.target_rate) {
			recommendations.push(
				`Improve D2 success rate from ${(
					d2Ready.success_rate * 100
				).toFixed(1)}% to ${(d2Ready.target_rate * 100).toFixed(1)}%`
			);
		}
	}
...
export function formatHandoffReport(report: HandoffReadinessReport): string {
	...
	const d1Status = d1.ready_for_handoff
		? "✅ READY for junior tier"
		: "⏳ NOT READY";
	lines.push(
		`  D1 Interventions: ${d1.count} types, ${(
			d1.success_rate * 100
		).toFixed(1)}% success - ${d1Status}`
	);

	const d2 = report.by_level.D2;
	const d2Status = d2.ready_for_handoff
		? "✅ READY for mid tier"
		: "⏳ NOT READY (target: 95%)";
	lines.push(
		`  D2 Interventions: ${d2.count} types, ${(
			d2.success_rate * 100
		).toFixed(1)}% success - ${d2Status}`
	);

	const d3 = report.by_level.D3;
	lines.push(
		`  D3 Interventions: ${d3.count} types, ${(
			d3.success_rate * 100
		).toFixed(1)}% success - Frontier only (by design)`
	);
	...
	for (const tier of report.tier_readiness) {
		...
		lines.push(
			...
			}): ${status} - ${(tier.success_rate * 100).toFixed(1)}%/${(
				tier.target_rate * 100
			).toFixed(1)}% success, ${tier.successful_runs}/${
				tier.required_runs
			} runs`
		);
		if (!tier.ready && tier.blocking_interventions.length > 0) {
			lines.push(
				`     Blocking: ${tier.blocking_interventions.join(", ")}`
			);
		}
	}
