 ...
 */
export function writeLockFile(lockFile: WeaveLockFile, workingDir: string = process.cwd()): string {
 ...
 */
export function readLockFile(workingDir: string = process.cwd()): WeaveLockFile | null {
  const lockFilePath = path.join(workingDir, LOCK_FILE_NAME);
...
export function validateLockFile(
  ...
  if (lockFile.planHash !== currentHash) {
    return {
      valid: false,
      reason:
        "Plan or PR heads have changed since lock file was created. " +
 ...
 */
export function updateLockFile(context: WeaveContext, workingDir: string = process.cwd()): string {
