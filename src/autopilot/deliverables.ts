  ...
  async updateLatestSymlink(deliverableDir: string): Promise<void> {
    ...
    try {
      ...
      if (process.platform === "win32") {
        console.warn("⚠️  Symlink failed, using directory copy fallback");
        console.warn("   (Windows requires Developer Mode or admin privileges for symlinks)");
  ...
  async cleanup(policy: RetentionPolicy): Promise<CleanupResult> {
    ...
    for (const deliverable of deliverables) {
      const dirName =
        ...
        `weave-${deliverable.timestamp.replace(/[:.]/g, "-").replace("Z", "")}`;
