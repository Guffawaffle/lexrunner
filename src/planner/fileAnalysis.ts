...
export class FileAnalyzer {
  ...
  async predictConflicts(
    ...
    for (let i = 0; i < prFileChanges.length; i++) {
      for (let j = i + 1; j < prFileChanges.length; j++) {
        ...
        if (sharedFiles.length > 0) {
          const severity = this.assessConflictSeverity(pr1.files, pr2.files, sharedFiles);
  ...
  private generateConflictReason(
    ...
    for (const filename of sharedFiles.slice(0, 3)) {
 ...
 */
export function createFileAnalyzer(octokit: Octokit, owner: string, repo: string): FileAnalyzer {
