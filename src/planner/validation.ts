 ...
 */
export function validatePlan(plan: Plan, options?: { verbose?: boolean }): ValidationResult {
  ...
  if (orphans.length > 0) {
    warnings.push({
      ...
      affectedPRs: orphans,
      suggestion:
        "Consider if these items should have dependencies or dependents. Use labels to mark intentional orphans.",
    });
  }
