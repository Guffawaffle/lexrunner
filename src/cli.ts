				...
				try {
					...
					try {
						...
						if (opts.json) {
							...
							console.error(
								`Tip: Check for syntax errors in the JSON file\n`
							);
						}
						throwExit(1);
					}
					...
					if (opts.migrate && needsMigration(data)) {
						try {
							...
							if (opts.json) {
								...
								console.log(
									`✓ ${file} migrated and validated successfully`
								);
								console.log(
		...
		try {
			...
			if (opts.dryRun && !opts.execute) {
				...
				if (opts.json || jsonModeActive) {
					...
					console.log(
						`🔍 DRY RUN MODE - Merge plan for ${plan.items.length} items → ${plan.target}`
					);
	...
	.action(async (opts) => {
		...
		if (fs.existsSync(smartergptDir)) {
			...
			const missingFiles = expectedFiles.filter(
				(file) => !fs.existsSync(path.join(smartergptDir, file))
			);
		...
		try {
			...
			console.log(
				`✓ Git: working directory ${isClean ? "clean" : "has changes"}`
			);
