...
export class EnterpriseAuditService {
  ...
  private signEntry(entry: SecureAuditEntry): string {
    if (!this.signingKey) {
      const axError = securityComplianceViolationError("Signing key not configured", {
        operation: "signEntry",
        requirement: "AUDIT_SIGNING_KEY environment variable",
      });
    ...
    return createHmac("sha256", this.signingKey).update(entry.hash).digest("hex");
  }
  ...
  generateComplianceReport(
    ...
    if (this.signingKey) {
      report.signature = createHmac("sha256", this.signingKey).update(report.content).digest("hex");
    }
  ...
  private generateSOC2Report(entries: AuditEntry[]): string {
    ...
    sections.push(
      `Audit Period: ${entries[0]?.timestamp || "N/A"} to ${entries[entries.length - 1]?.timestamp || "N/A"}`
    );
    ...
    const changeEvents = entries.filter(
      (e) =>
  ...
  trimRetention(
    ...
    const supportedFrameworks = Array.from(
      new Set(Array.from(lookup.values()).map((v) => v.display))
    );
  ...
  private normalizeFramework(
    input: string
  ): { display: string; minDays: number; original: string } | null {
    const lookup = this.ensureFrameworkLookup();
  ...
  private ensureFrameworkLookup(): Map<
    string,
    { display: string; minDays: number; original: string }
  > {
    ...
    for (const rec of this.getRetentionRecommendations()) {
      // Primary token
      map.set(this.frameworkToken(rec.framework), {
        display: rec.framework,
        minDays: rec.minDays,
        original: rec.framework,
      });
      // Known aliases
      ...
      for (const alias of aliasList) {
        map.set(this.frameworkToken(alias), {
          display: rec.framework,
          minDays: rec.minDays,
          original: rec.framework,
        });
