...
export class SecretsManager {
  ...
  async requireSecret(id: string): Promise<string> {
    ...
    if (!secret) {
      const axError = securitySecretNotFoundError(`Required secret '${id}' not found`, {
        secretId: id,
      });
  ...
  scanText(content: string): DetectedSecret[] {
    ...
    for (const pattern of this.patterns) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (/private|secret|token|key/i.test(pattern.name))
          confidence = Math.min(1, confidence * 1.1 + 0.1);
