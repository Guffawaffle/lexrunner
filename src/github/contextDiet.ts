...
import { extractDiffHunks, optimizeHunkSize, formatMinimalDiff, calculateContextMetrics, type MinimalDiffContext, type ContextMetrics } from "./diffHunks.js";
import { buildSymbolMaps, formatSymbolMaps, type SymbolMapContext } from "./symbolMap.js";
import { filterPRMetadata, filterPRsWithMetrics, formatMinimalPR, formatMinimalPRs, type MinimalPRMetadata, type MinimalPRContext } from "./minimalContext.js";
...
export async function buildMinimalContext(
	...
	const estimatedOriginalSize = 
		prMetadata.originalSize + 
		(diffMetrics?.rawSize || 0) + 
		(symbolMapSize || 0); // Symbol maps don't have reduction

	const reductionPercent = estimatedOriginalSize > 0
