...
export function formatMinimalDiff(context: MinimalDiffContext): string {
  ...
  parts.push(
    `# Diff Context (${context.fileCount} files, ${context.hunkCount} hunks, ${context.totalSize} chars)`
  );
  parts.push("");

  for (const file of context.files) {
    parts.push(
      `## ${file.changeType}: ${file.path}${file.previousPath ? ` (from ${file.previousPath})` : ""}`
    );
...
export function calculateContextMetrics(
  ...
  const avgHunkSize =
    minimalContext.hunkCount > 0 ? minimalContext.totalSize / minimalContext.hunkCount : 0;
