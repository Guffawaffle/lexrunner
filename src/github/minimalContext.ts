 ...
 */
const CONTEXT_LABELS = ["needs-context", "needs-body", "full-context"];
...
export function shouldIncludeBody(labels: Array<{ name: string }>): boolean {
  return labels.some((label) =>
    CONTEXT_LABELS.some((contextLabel) => label.name.toLowerCase() === contextLabel.toLowerCase())
  );
}
...
export function filterPRsWithMetrics(prs: PullRequest[]): MinimalPRContext {
  ...
  const reductionPercent = originalSize > 0 ? ((originalSize - totalSize) / originalSize) * 100 : 0;
...
export function formatMinimalPRs(context: MinimalPRContext): string {
  ...
  parts.push(
    `Size: ${context.totalSize} chars (${context.reductionPercent.toFixed(1)}% reduction)`
  );
  parts.push("");
...
export function markNeedsContext(pr: PullRequest): PullRequest {
  ...
  return {
    ...pr,
    labels: [...pr.labels, { name: "needs-context", color: "d4c5f9" }],
  };
}
