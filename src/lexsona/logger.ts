  ...
  for (const file of files) {
    try {
      ...
      if (rawLog.schemaVersion) {
        ...
        wasLegacy = true;
        warnings.push("missing schemaVersion - normalized to 0.0.0 (legacy)");
...
export function formatGovernanceLog(log: GovernanceComparisonLog): string {
  ...
  if (log.lexsona.constraintSet) {
    lines.push(`  Constraints: ${log.lexsona.constraintSet.constraintCount}`);
    ...
    if (log.lexsona.constraintSet.topConstraints.length > 0) {
      ...
      for (const c of log.lexsona.constraintSet.topConstraints.slice(0, 5)) {
        lines.push(`    - [${c.severity}] ${c.description} (${(c.confidence * 100).toFixed(0)}%)`);
      }
    }
  }
  lines.push("");
  ...
  if (log.runner.hostilityScore !== undefined) {
    lines.push(`  Hostility Score: ${log.runner.hostilityScore.toFixed(2)}`);
  }
 ...
 * @returns Number of logs deleted
 */
export async function enforceRetentionPolicy(maxSizeMB: number = 100): Promise<number> {
  const logsDir = getGovernanceLogsDir();
  if (!existsSync(logsDir)) {
    return 0;
  }
 ...
 */
export async function applyRetentionPolicy(config: RetentionConfig = {}): Promise<void> {
