...
type LexSonaInstance = {
  deriveConstraints: (ctx: LexSonaDeriveContext) => Promise<LexSonaConstraintSet>;
};
...
async function getLexSonaModule(): Promise<LexSonaModuleLike | null> {
  ...
  try {
    ...
    LexSonaModule = (await import(moduleName)) as unknown as LexSonaModuleLike;
    return LexSonaModule;
  } catch {
    return null;
  }
...
function toConstraintSnapshot(constraintSet: {
  ...
  const sorted = [...constraintSet.constraints].sort((a, b) => b.confidence - a.confidence);
...
export async function deriveShadowConstraints(
  ...
  try {
    ...
    debugLog(`  Lex DB: ${process.env.LEX_DB_PATH ?? "(none - offline mode)"}`);
    ...
    debugLog(`  Rules considered: ${constraintSet.metadata.rulesConsidered}`);
    debugLog(`  Rules filtered: ${constraintSet.metadata.rulesFiltered}`);
    debugLog(`  Confidence threshold: ${constraintSet.metadata.confidenceThreshold}`);
    ...
    if (constraintSet.constraints.length > 0) {
      ...
      for (const c of constraintSet.constraints) {
        debugLog(`  - [${c.severity}] ${c.text} (confidence: ${(c.confidence * 100).toFixed(0)}%)`);
      }
    } else {
...
export function formatShadowGovernanceSummary(
  ...
  if (!shadowResult.success) {
    const prefix = opts.noColor ? "[LexSona shadow]" : chalk.yellow("[LexSona shadow]");
    lines.push(`${prefix} ${shadowResult.personaId ?? "unknown"}: ERROR - ${shadowResult.error}`);
    return lines.join("\n");
  }
  ...
  // Main status line
  const prefix = opts.noColor ? "[LexSona shadow]" : chalk.blue("[LexSona shadow]");
  ...
  if (constraintCount > 0 && shadowResult.constraintSet) {
    const topConstraints = shadowResult.constraintSet.topConstraints.slice(0, 3);
    for (const constraint of topConstraints) {
      const confStr = (constraint.confidence * 100).toFixed(0);
      lines.push(`  - "${constraint.description.slice(0, 60)}" (confidence: ${confStr}%)`);
    }
  ...
  if (shadowResult.offlineMode && shadowResult.constraintSet?.metadata.offlineMode) {
