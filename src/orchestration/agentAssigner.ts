  ...
  while (retries < maxRetries) {
    try {
      ...
      if (
        ...
        error.message?.includes("Agent already exists")
      ) {
        return {
          issueNumber,
          status: "skipped",
...
export async function assignAgentsToBatch(
  ...
  for (let i = 0; i < issues.length; i++) {
    ...
    const result = await assignAgentWithRetry(issueNumber, options.repo, assignFn);
...
export function formatAssignmentLog(log: AssignmentLog, repo: string): string {
  ...
  for (const result of log.results) {
    const prefix = result.status === "success" ? "✅" : result.status === "skipped" ? "⏭️" : "❌";
  ...
  lines.push("");
  lines.push(
    `Summary: ${log.summary.assigned} assigned, ${log.summary.skipped} skipped, ${log.summary.failed} failed`
  );
