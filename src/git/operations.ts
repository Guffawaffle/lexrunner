...
export class GitOperations {
  ...
  async createWeaveBranch(baseBranch: string = "main"): Promise<string> {
    ...
    try {
      ...
      throw new GitOperationError(
        `Failed to create weave branch: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
    ...
    try {
          ...
          case "rebase-weave":
            ...
            try {
              ...
              return {
                ...
                item,
                conflicts:
                  conflictedFiles.length > 0 ? conflictedFiles : ["Rebase conflicts detected"],
      ...
      if (hasMergeFailure || conflictedFiles.length > 0) {
        ...
        emitFailureReceipt(
          ...
          { log: false }
        ); // Don't log to avoid cluttering output
  ...
  async executeWeave(
    ...
    try {
      ...
      emitActionReceipt(
        {
          ...
          escalationReason:
            outcome !== "success" ? "Weave execution did not fully succeed" : undefined,
        },
        { log: false }
      ); // Don't log to avoid cluttering output
      ...
      throw new GitOperationError(
        `Weave execution failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
    ...
    try {
      ...
      for (const branch of weaveBranches) {
        try {
          ...
          console.warn(
            `Could not delete branch ${branch}: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    } catch (error) {
  ...
  async getCurrentHead(): Promise<string> {
    try {
      ...
      throw new GitOperationError(
        `Failed to get current HEAD: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async resetHard(targetSha: string): Promise<void> {
    try {
      ...
      throw new GitOperationError(
        `Failed to reset to ${targetSha}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
