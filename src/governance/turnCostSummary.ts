 ...
 */
export const TURN_COST_REGRESSION_THRESHOLD = 0.20;
...
export function formatTurnCostSummary(summary: TurnCostSummary): string {
	...
	if (summary.priorRunScore !== undefined) {
		...
		if (comparison.isRegression) {
			...
			lines.push(`   Current score is ${(comparison.percentageChange! * 100).toFixed(0)}% higher than prior run`);
		}
	}
 ...
 */
export function compareTurnCosts(
	currentScore: number,
	priorScore?: number
): TurnCostComparison {
...
export function formatTurnCostComparison(comparison: TurnCostComparison): string {
	...
	lines.push("");
	lines.push(comparison.isRegression
...
export function turnCostSummaryToJSON(
	...
	return {
		...
		comparison: priorScore !== undefined ? {
			priorScore,
