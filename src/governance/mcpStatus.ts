...
export function buildGovernanceStatus(
	...
	if (turnCostSummary) {
		...
		if (priorTurnCostScore !== undefined && priorTurnCostScore > 0) {
			...
			hasRegression = percentageChange > 0.20; // 20% threshold
	...
	if (hostilityScore) {
		...
		if (hostilityScore.status === "high") {
			warnings.push(
				`Environment hostility is high (${(hostilityScore.total * 100).toFixed(0)}%)`
			);
...
export function formatGovernanceStatus(status: GovernanceStatus): string {
	...
	if (status.tiers) {
		...
		lines.push("");
		lines.push(`Escalation Rate: ${(status.tiers.escalationRate * 100).toFixed(1)}%${status.tiers.escalationWarning ? " ⚠️" : ""}`);
	...
	if (status.hostility) {
		const hostilityIcon =
			status.hostility.status === "low"
				? "✅"
				: status.hostility.status === "medium"
					? "⚠️"
					: "❌";
