...
export class IssueAnalyzer {
	 ...
	 */
	private computeComplexity(body: string, affectedFiles: string[]): {
	 ...
	 */
	private extractCopilotAgent(body: string, assignees: Array<{ login: string }>): {
		assigned: boolean;
		agent?: string;
	} {
	 ...
	 * Estimate duration
	 */
	private estimateDuration(complexity: { score: number }, body: string): {
		hours: number;
	...
	computeOverlap(issue1: IssueMetadata, issue2: IssueMetadata): OverlapScore {
		...
		// Label overlap (0-1)
		const labelOverlap = this.jaccardIndex(
			new Set(issue1.labels),
			new Set(issue2.labels)
		);
	...
	identifyParallelGroups(
		...
		for (const issue of metadata) {
			...
			if (group.length > 1) {
				...
				const avgOverlap = groupOverlaps.length > 0
