...
export async function prepareReviewContext(
  ...
  const outputDir = input.output_dir ?? `/tmp/senior-dev-review-${pr_number}-${timestamp}`;
  ...
  if (prMetadataResult.success) {
    ...
    fs.writeFileSync(prMetadataPath, JSON.stringify({ error: "PR not found or gh CLI error" }));
    commandsRun.push({
      command: "gh pr view --json",
      status: "error",
  ...
  fs.writeFileSync(prDescPath, prDescResult.output || "# No description available");
  ...
  fs.writeFileSync(diffPath, diffResult.output || "# Could not generate diff");
  commandsRun.push({
    command: "git diff",
  ...
  // 2b. Diff Summary
  const diffSummaryResult = runCommand("git", ["diff", "--stat", `${base_branch}...HEAD`], { cwd });
  const diffSummaryPath = path.join(outputDir, "changes-summary.txt");
  fs.writeFileSync(diffSummaryPath, diffSummaryResult.output || "# No summary");

  // 3. Changed Files
  const changedFilesResult = runCommand("git", ["diff", "--name-only", `${base_branch}...HEAD`], {
    cwd,
  });
  ...
  if (input.skip_typecheck) {
    ...
    fs.writeFileSync(typecheckPath, typecheckResult.output || "# No typecheck output");
    commandsRun.push({
  ...
  for (const module of modulesDetected.slice(0, 3)) {
    const recallResult = runCommand("lex", ["recall", `reviews for ${module}`], { cwd });
...
export async function recallSeniorDevContext(
  ...
  switch (query_type) {
    case "module": {
      ...
      const result = runCommand("lex", ["recall", `reviews for ${query}`]);
      if (result.success && result.output) {
        // Parse frames from output (simplified - real implementation would parse JSON)
        frames.push(...parseFramesFromLexOutput(result.output, query_type));
      }

      // Also get related patterns
      const patternResult = runCommand("lex", ["list_frames", "--keyword", "pattern"]);
      if (patternResult.success && patternResult.output) {
        const allPatterns = parseFramesFromLexOutput(patternResult.output, "pattern");
        relatedFrames.push(
          ...allPatterns.filter((f) =>
            f.reference_point.toLowerCase().includes(query?.toLowerCase() ?? "")
          )
        );
      }
    ...
    case "developer": {
      ...
      const result = runCommand("lex", ["recall", "--keyword", `developer:${query}`]);
      if (result.success && result.output) {
        frames.push(...parseFramesFromLexOutput(result.output, query_type));
      }

      // Also get learning path
      const pathResult = runCommand("lex", ["recall", `Developer ${query} learning path`]);
      if (pathResult.success && pathResult.output) {
        relatedFrames.push(...parseFramesFromLexOutput(pathResult.output, "learning-path"));
      }
    ...
    case "pattern": {
      ...
      const result = runCommand("lex", ["list_frames", "--keyword", "pattern"]);
      if (result.success && result.output) {
        frames.push(...parseFramesFromLexOutput(result.output, query_type));
      }
      ...
      if (reviewResult.success && reviewResult.output) {
        relatedFrames.push(...parseFramesFromLexOutput(reviewResult.output, "review"));
      }
  ...
  return {
    ...
    related_frames: relatedFrames.length > 0 ? relatedFrames.slice(0, 5) : undefined,
...
export async function captureSeniorDevFrame(input: CaptureFrameInput): Promise<CaptureFrameResult> {
  ...
  if (result.success) {
    ...
    const idMatch = result.output.match(/frame[_-]?id[:\s]+([a-zA-Z0-9-]+)/i);
    if (idMatch) {
      frameId = idMatch[1];
    }
  } else {
