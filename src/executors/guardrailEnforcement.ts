...
import type { GuardrailProfile, G_scope, AccessPattern } from "../types/guardrails.js";
 ...
 */
export function matchesAccessPattern(filePath: string, pattern: AccessPattern): boolean {
...
export function checkFileAccess(
  ...
  for (const denyPattern of scope.files.deny ?? []) {
    if (denyPattern.access === accessType && matchesAccessPattern(filePath, denyPattern)) {
  ...
  if (scope.files.allow && scope.files.allow.length > 0) {
    ...
    for (const allowPattern of scope.files.allow) {
      if (allowPattern.access === accessType && matchesAccessPattern(filePath, allowPattern)) {
        allowed = true;
        break;
      }
    }
 ...
 * @returns Enforcement result
 */
export function checkNetworkAccess(host: string, scope: G_scope | undefined): EnforcementResult {
  const violations: GuardrailViolation[] = [];
 ...
 * @returns Enforcement result
 */
export function checkEnvAccess(envVar: string, scope: G_scope | undefined): EnforcementResult {
  const violations: GuardrailViolation[] = [];
