...
import { toolBudgetExceededError, type AXError } from "../errors/index.js";
...
export class ToolBudgetEnforcer {
  ...
  checkToolCall(
    ...
    if (this.budget.denied.includes(toolName)) {
      return {
        allowed: false,
        error: toolBudgetExceededError(`Tool '${toolName}' is denied by executor budget`, {
          tool: toolName,
    ...
    if (this.budget.allowed.length > 0 && !this.budget.allowed.includes(toolName)) {
      return {
        allowed: false,
        error: toolBudgetExceededError(`Tool '${toolName}' is not in the allowed tool list`, {
          tool: toolName,
    ...
    if (this.budget.limits?.maxTokensOut !== undefined && estimatedTokensOut > 0) {
      const nextTokenCount = this.state.totalTokensOut + estimatedTokensOut;
  ...
  getBudget(): Readonly<ToolBudget> {
    return {
      allowed: [...this.budget.allowed],
      denied: [...this.budget.denied],
      limits: this.budget.limits ? { ...this.budget.limits } : undefined,
    };
  }
