...
export class ProcedureLoader {
  ...
  async loadProcedure(id: string): Promise<Procedure> {
    ...
    if (!fs.existsSync(filePath)) {
      throw new ProcedureLoadError(`Procedure "${id}" not found at ${filePath}`, id, filePath);
    }
 ...
 */
export function createProcedureLoader(options?: ProcedureLoaderOptions): ProcedureLoader {
