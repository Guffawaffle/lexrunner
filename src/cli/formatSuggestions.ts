...
export function formatSuggestionsAsTable(
  ...
  for (const suggestion of suggestions) {
    ...
    if (suggestion.evidence.explicit && suggestion.evidence.explicit.length > 0) {
      ...
      lines.push(
        `  Evidence: ${fileCount} shared file${fileCount > 1 ? "s" : ""} ${dirExample ? `(${dirExample})` : ""}`
      );
    }
  ...
  lines.push(
    `Total: ${suggestions.length} suggestion${suggestions.length > 1 ? "s" : ""} (${highConfidence} high-confidence${lowConfidence > 0 ? `, ${lowConfidence} low-confidence` : ""})`
  );
  ...
  if (highConfidence.length > 0) {
    ...
    for (const suggestion of highConfidence) {
      ...
      if (suggestion.evidence.explicit && suggestion.evidence.explicit.length > 0) {
        ...
        const dirExample = extractDirectory(suggestion.evidence.files[0]);
        lines.push(
          `- **Evidence:** ${fileCount} shared file${fileCount > 1 ? "s" : ""} ${dirExample ? `in \`${dirExample}\`` : ""}`
        );
      }
