 ...
 */
export function formatStatusTable(results: Map<string, any>, mergeSummary: any): string {
  const lines: string[] = [];
  ...
  for (const [name, result] of results) {
    ...
    lines.push(
      `| ${name} | ${statusIcon} ${result.status} | ${gateCount} | ${eligible} | ${gateDetails} |`
    );
  }
  ...
  lines.push("### Summary");
  lines.push(
    `- **Eligible**: ${mergeSummary.eligible.length} ${pluralize(mergeSummary.eligible.length, "node")} ready for merge`
  );
  lines.push(
    `- **Pending**: ${mergeSummary.pending.length} ${pluralize(mergeSummary.pending.length, "node")} waiting`
  );
  lines.push(
    `- **Failed**: ${mergeSummary.failed.length} ${pluralize(mergeSummary.failed.length, "node")} with failures`
  );
  lines.push(
    `- **Blocked**: ${mergeSummary.blocked.length} ${pluralize(mergeSummary.blocked.length, "node")} blocked by dependencies`
  );
...
export function formatCSV(data: Record<string, any>[]): string {
  ...
  const rows = data.map((item: any) => headers.map((h) => JSON.stringify(item[h] || "")).join(","));
