...
export class SnapshotBuilder {
  ...
  private async extractHunkForTarget(
    ...
    const isFailureFile =
      this.normalizeFilePath(failureFileRel) === this.normalizeFilePath(targetFileRel);

    let startLine = 1;
    let endLine = lines.length;

    if (isFailureFile && typeof failureLine === "number" && Number.isFinite(failureLine)) {
   ...
   */
  private buildScope(targetFiles: string[], overrides?: Partial<ScopeBoundary>): ScopeBoundary {
