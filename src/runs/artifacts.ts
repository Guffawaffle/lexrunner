...
export const ListArtifactsInputSchema = z.object({
  ...
  path: z.string().optional().describe("Filter by path pattern (supports * and ** wildcards)"),
  latestOnly: z.boolean().optional().describe("Only return the most recent artifact of each type"),
  inline: z.boolean().optional().describe("Include content for small artifacts (< 10KB)"),
});
...
export const ArtifactDescriptorSchema = z.object({
  ...
  type: z.enum(["plan", "decision", "failure", "gate", "report", "log"]).describe("Artifact type"),
