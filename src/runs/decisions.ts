 ...
 */
function findActionOption(action: string, nextOptions: NextOption[]): NextOption | null {
...
export function submitDecision(
  ...
  if (!actionOption) {
    const availableActions = currentStatus.nextOptions.map((opt) => opt.action).join(", ");
    return {
      accepted: false,
  ...
  const schemaValidation = validateResponseSchema(response, actionOption.responseSchema);
 ...
 */
export function getDecisions(runId: string, baseDir: string = process.cwd()): DecisionLogEntry[] {
  try {
