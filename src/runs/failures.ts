...
export type FailureErrorCode = (typeof FailureErrorCode)[keyof typeof FailureErrorCode];
...
export type FailureHandlingPayload = z.infer<typeof FailureHandlingPayloadSchema>;
 ...
 */
function buildHint(code: FailureErrorCode, gateName: string, runId?: string): string {
...
export function wrapGateFailure(
  ...
  // Classify the error
  const { code, retryable } = classifyGateError(gateName, stderr, exitCode, options?.isFlaky);
 ...
 */
export function getGateFailures(runId: string, baseDir: string = process.cwd()): FailureRecord[] {
...
export function failurePayloadToAXError(
  ...
  const nextActions = payload.recommendedActions.map((action) => action.description);
  ...
  return createAXError(payload.error.code, payload.error.message, nextActions, context);
}
