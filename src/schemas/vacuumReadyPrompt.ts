...
export const ToolAnchoring = z
  .object({
    allowedTools: z.array(z.string()).min(1, "At least one tool must be specified"),
    maxToolCalls: z.number().int().min(1).optional(),
    rationale: z.string().min(1, "Tool selection rationale is required").optional(),
  })
  .strict();
...
export const TaskAtomicity = z
  .object({
    ...
    deliverableType: z.enum(["frame", "report", "decision", "artifact", "analysis"]),
 ...
 */
export function validateVacuumReadyPrompt(data: unknown): VacuumReadyPrompt {
 ...
 */
export function safeParseVacuumReadyPrompt(data: unknown): ZodSafeParseResult<VacuumReadyPrompt> {
