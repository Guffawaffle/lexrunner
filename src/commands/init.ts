	...
	if (!options.nonInteractive) {
		...
		if (githubToken) {
			try {
				...
				console.log(
					`✓ GitHub: Connected to ${repoInfo.owner}/${repoInfo.repo}\n`
				);

				// Store token suggestion
				console.log(
					"💡 Tip: Store your GitHub token in an environment variable:"
				);
				console.log("   export GITHUB_TOKEN=your_token_here\n");
			} catch (error) {
				if (error instanceof GitHubAuthError) {
					console.log(
						"✗ GitHub authentication failed. Continuing without GitHub integration.\n"
					);
				} else {
					console.log(
						`ℹ GitHub: ${
