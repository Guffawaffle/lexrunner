...
export function registerCreateProjectCommand(program: Command): void {
	program
		...
		.option('--issue-labels <labels>', 'Additional sub-issue labels (comma-separated)', parseLabels, [])
...
function parseLabels(value: string): string[] {
	return value.split(',').map(l => l.trim()).filter(l => l.length > 0);
}
...
async function createIssuesOnGitHub(
	...
	if (options.project) {
		...
		console.warn(chalk.yellow('⚠ GitHub Projects linking not yet implemented (requires GraphQL API)'));
	}

	console.log(chalk.green(`\n✓ Project created successfully!`));
	console.log(chalk.gray(`  Epic: #${epic.number}`));
	console.log(chalk.gray(`  Sub-Issues: ${Array.from(subIssueMap.values()).map(n => `#${n}`).join(', ')}`));
}
...
function formatSubIssueBody(subIssue: SubIssue, epicNumber: number): string {
	return (
		...
		(subIssue.dependsOn.length > 0 ? `## Dependencies\n\n${subIssue.dependsOn.map(d => `- ${d}`).join('\n')}\n\n` : '') +
