		...
		.action(async (opts) => {
			try {
				...
				if (!githubAPI) {
					...
					console.error(
						"     lex-pr weave discover --owner <owner> --repo <repo>\n"
					);
					throwExit(1);
				}
		...
		.action(async (opts) => {
			try {
				...
				const plan = await generatePlanFromGitHub(client, {
					query: opts.query,
					labels: opts.labels,
					excludePRs: opts.excludePrs?.map((n: string) =>
						parseInt(n, 10)
					),
				...
				if (opts.json || deps.jsonModeActive()) {
					...
					console.log(
						`  1. Review: lex-pr plan-review ${opts.output}`
					);
					console.log(
						`  2. Dry-run: lex-pr weave apply --plan ${opts.output} --dry-run`
					);
					console.log(
						`  3. Execute: lex-pr weave apply --plan ${opts.output}\n`
					);
				}
			} catch (error) {
				const message =
			...
			try {
				...
				if (opts.dryRun) {
					...
					if (opts.json || deps.jsonModeActive()) {
						...
						console.log("\nTo execute:");
						console.log(
							`  lex-pr weave apply --plan ${opts.plan}\n`
						);
					}
				} else {
					// Execute mode
					...
					if (!opts.json && !deps.jsonModeActive()) {
						...
						console.log("\nNext step:");
						console.log(
							`  lex-pr merge --plan ${opts.plan} --execute\n`
						);
					} else {
						console.log(
							canonicalJSONStringify({
