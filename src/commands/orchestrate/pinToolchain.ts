...
import { verifyToolchainPins, ToolVersion } from "../../orchestration/determinism.js";
 ...
 */
function formatToolVersion(tool: ToolVersion, useColor: boolean = true): string {
...
export function registerPinToolchainCommand(program: Command): void {
	const orchestrateCmd = program.command("orchestrate").description("Orchestration commands for merge-weave operations");
	
	orchestrateCmd
		.command("pin-toolchain")
		.description("Verify toolchain versions match pinned versions")
		...
		.action(async (opts: PinToolchainOptions, command: Command) => {
			...
			try {
				...
				if (jsonMode) {
					...
					if (allMatch) {
						console.log("✅ All toolchain versions match pinned versions");
					} else {
						console.log("❌ Some toolchain versions do not match pinned versions");
						console.log("\n💡 Tip: Install pinned versions using asdf, mise, nvm, or volta");
						console.log("   See docs/determinism.md for setup instructions");
					}
				}
