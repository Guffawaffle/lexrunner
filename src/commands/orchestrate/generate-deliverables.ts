...
export function registerGenerateDeliverablesCommand(program: Command): void {
	program
		...
		.description("Generate merge-weave deliverables with plan hash and toolchain manifest")
		...
		.action(async (options) => {
			try {
				...
				if (!options.batch) {
					...
					console.error("Example: lex-pr orchestrate:generate-deliverables --batch batch3 --plan plan.json");
					process.exit(1);
				}

				// Determine output directory
				const outputDir = options.outputDir || 
					path.join(".smartergpt.local", "deliverables", options.batch);
				...
				console.log(`✅ Created: ${path.join(outputDir, "plan-hash.txt")}`);
				console.log(`✅ Created: ${path.join(outputDir, "toolchain-manifest.json")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE0_preflight.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE1_assignment.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE2_conflicts.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE3_merge.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE4_gates.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "GATE5_cleanup.md")}`);
				console.log(`✅ Created: ${path.join(outputDir, "SUMMARY.md")}`);
				console.log("");

				// Read and display plan hash
				const fs = await import("fs");
				const planHashContent = fs.readFileSync(path.join(outputDir, "plan-hash.txt"), "utf-8");
