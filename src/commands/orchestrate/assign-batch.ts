...
import { assignAgentsToBatch, formatAssignmentLog, AssignmentLog } from "../../orchestration/index.js";
...
async function getRepository(): Promise<string> {
	...
	throw new Error('Could not determine repository. Set GITHUB_REPOSITORY environment variable or run in a git repository.');
}
...
export function registerAssignBatchCommand(program: Command): void {
	const assignBatch = program
		...
		.description('Bulk-assign GitHub Copilot agents to batched issues')
		...
		.action(async (options, command) => {
			...
			try {
				...
				if (isJsonMode) {
					console.error(canonicalJSONStringify({
