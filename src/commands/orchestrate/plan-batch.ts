...
export function registerPlanBatchCommand(
	program: Command,
	...
	program
		...
		.description(
			"Generate batch plan using Kahn's algorithm (canonical: lex-pr weave plan)"
		)
		.option(
			"--issues <numbers>",
			"Comma-separated issue/PR numbers (e.g., 156,157,160)"
		)
			...
			try {
				...
				if (options.input) {
					...
					if (Array.isArray(inputData)) {
						...
						throw new Error(
							"Invalid input format: expected nodes, issues array, or direct array"
						);
					}
				}
				...
				const isJsonMode =
					options.json || (jsonModeActive && jsonModeActive());
				...
				if (isJsonMode) {
					...
					for (const batch of plan.batches) {
						...
						for (const item of batch.items) {
							...
							console.log(
								`  - ${itemId}: score ${item.metadata.score.toFixed(
									1
								)}${deps}`
							);
						}
					...
					console.log(
						chalk.gray(`Deterministic: ${plan.deterministic}`)
					);
				}
				...
				if (error instanceof CycleError) {
					console.error(
						chalk.red("Error: Cycle detected in dependency graph")
					);
