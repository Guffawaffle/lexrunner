...
export function createGovernanceCleanupCommand(): Command {
	...
	cmd.description("Clean up old governance logs based on retention policy")
		.option(
			"--max-age <days>",
		...
		.option(
			"--max-size <mb>",
			"Delete oldest logs if total size exceeds this (MB)",
			"100"
		)
		.option(
			"--dry-run",
		...
		.action(
			async (opts: {
				...
				if (opts.stats) {
					...
					if (stats.oldestDate) {
						console.log(
							`  Oldest log: ${stats.oldestDate.toISOString()}`
						);
					}
					if (stats.newestDate) {
						console.log(
							`  Newest log: ${stats.newestDate.toISOString()}`
						);
					}
					console.log("");
					return;
				...
				if (opts.dryRun) {
					...
					if (wouldDeleteSize > 0) {
						console.log(
							`Would delete ${wouldDeleteSize} logs to meet size limit`
						);
					}
 ...
 */
export function registerGovernanceCleanupCommand(
