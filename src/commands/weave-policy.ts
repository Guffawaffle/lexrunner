...
import { loadPolicy, loadPolicyOrNull, PolicyLoadError } from "../weave/policy/index.js";
import {
 ...
 */
export function registerPolicyCommands(weaveCommand: Command, deps: PolicyCommandDeps): void {
...
`
    ...
    .action(async (opts) => {
      try {
        ...
        if (opts.json || deps.jsonModeActive()) {
          ...
          console.log(`\n📦 Discovery`);
          console.log(`   Repos: ${result.policy.discovery.repos.length}`);
          for (const repo of result.policy.discovery.repos) {
            console.log(`     - ${repo.owner}/${repo.name} (priority: ${repo.priority})`);
          }
          if (result.policy.gates?.base_branch?.required) {
            ...
            console.log(
              `   Per-PR CI: ${
                result.policy.gates?.per_pr?.require_ci_green ? "required" : "optional"
              }`
            );
          }
          if (result.policy.merge) {
            ...
            console.log(
              `   Admin authority: ${
                result.policy.merge.admin_authority?.enabled ? "enabled" : "disabled"
              }`
            );
          }
  ...
  policy
    ...
    .action(async (opts) => {
      try {
        ...
        console.log(`   Schema version: ${result.policy.schemaVersion}`);
        console.log(`   Repos: ${result.policy.discovery.repos.length}`);
        const gateCount = result.policy.gates?.base_branch?.required?.length ?? 0;
    ...
    .action(async (opts) => {
      try {
        ...
        if (opts.json || deps.jsonModeActive()) {
          ...
          console.log(`     D1 (scriptable): ${plan.stats.byDeterminism.D1}`);
          console.log(`     D2 (bounded):    ${plan.stats.byDeterminism.D2}`);
          console.log(`     D3 (frontier):   ${plan.stats.byDeterminism.D3}`);
          console.log(`   By phase:`);
          for (const [phase, count] of Object.entries(plan.stats.byPhase)) {
            console.log(`     ${phase}: ${count}`);
          }

          if (opts.output) {
            const fs = await import("node:fs/promises");
            await fs.writeFile(opts.output, JSON.stringify(plan, null, 2), "utf-8");
  ...
  policy
    ...
    .option("--output-dir <path>", "Directory for artifacts", ".smartergpt/deliverables")
    .action(async (opts) => {
      try {
        const result = await loadPolicy({ policyPath: opts.file });
        ...
        // Create audit logger
        const auditPath = resolve(outputDir, `merge-weave-${runId}.ndjson`);
        ...
        if (opts.d1Only) {
          interventions = interventions.filter((i) => hasD1Handler(i.type));
        }
        ...
        if (opts.dryRun) {
          ...
          for (const i of interventions) {
            console.log(`   [${i.determinism}] ${i.type} → ${i.target ?? "base"}`);
          }
          ...
          console.log(`   (Execution engine ready - requires API integration)`);
        }
