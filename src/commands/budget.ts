 ...
 */
export function registerBudgetCommand(program: Command, jsonModeActive: () => boolean): void {
  ...
  • Escalations: Number of tier escalations allowed`
    ...
    .action((opts) => {
      try {
        ...
        console.error(
          `Error displaying budget: ${error instanceof Error ? error.message : String(error)}`
        );
        throwExit(1);
      }
    });
  ...
  $ lex-pr budget reset --json            # JSON output after reset`
    ...
    .action((opts) => {
      try {
        ...
        console.error(
          `Error resetting budget: ${error instanceof Error ? error.message : String(error)}`
        );
        throwExit(1);
      }
    });
  ...
  Senior: tokens=15000, turns=10, time=600000ms, escalations=3`
    ...
    .action((opts) => {
      try {
        ...
        console.error(
          `Error setting budget: ${error instanceof Error ? error.message : String(error)}`
        );
        throwExit(1);
      }
    });
