 ...
 *
 * Commands for managing fanout templates:
...
function formatTemplatesList(templates: FanoutTemplates): string {
	...
	const lines: string[] = [
		`Fanout Templates (version ${templates.version})`,
		"=".repeat(50),
		"",
	];
...
export function registerFanoutCommands(weaveCommand: Command): void {
	const fanout = weaveCommand
		.command("fanout")
		.description("Manage fanout templates for follow-up issue creation");
		...
		.action(async (options: { file?: string }) => {
			try {
				...
				if (options.file) {
					...
					const result = await discoverFanoutTemplates(
						getWorkspaceRoot()
					);
					if (!result) {
						console.log(
							"No fanout templates found in default locations."
						);
						console.log(
							"Create .smartergpt/fanout-templates.yml to get started."
						);
						return;
					}
		...
		.action(
			async (options: {
				...
				try {
					...
					console.log(
						`Scanning PR #${options.pr} in ${repo} for fanout triggers...`
					);
					console.log(
						"(Note: Actual PR scanning requires GitHub API integration)"
					);
					console.log("");
					...
					console.log("");
					console.log(
						"To fully implement scanning, integrate with GitHub PR files API."
					);
				} catch (error) {
					console.error(
						`Error: ${
...
`,
					...
					const matches = matchTemplatesAgainstDiff(
						templates,
						sampleFiles
					);
					...
					console.log(
						"Use --sample to see a demonstration with sample data."
					);
					console.log("");
