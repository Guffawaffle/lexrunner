 ...
 */
export function registerSeniorDevCommand(program: Command, jsonModeActive: () => boolean): void {
  ...
  seniorDev
    ...
    .option("-l, --limit <number>", "Maximum number of frames to return", "10")
    .option("--json", "Output as JSON only")
    .action(async (queryType: string, query: string | undefined, options) => {
      // Validate query type
      const validTypes: RecallQueryType[] = ["module", "developer", "pattern", "pr"];
      ...
      if (queryType !== "pattern" && !query) {
        console.error(`Query type '${queryType}' requires a query argument`);
        process.exit(1);
      }
  ...
  seniorDev
    ...
    .option("-d, --developer <name>", "Developer name for tracking", "unknown")
    .option(
      "-s, --severity <level>",
    ...
    .option("--json", "Output as JSON only")
    .action(
      async (prNumber: string, module: string, summary: string, nextAction: string, options) => {
        const input: CaptureFrameInput = {
          ...
          jira: options.jira,
          blockers: options.blockers ? options.blockers.split(",") : undefined,
        };
...
function printPrepareContextResult(result: Awaited<ReturnType<typeof prepareReviewContext>>): void {
  console.log();
  console.log("╔══════════════════════════════════════════════════════════════════════╗");
  console.log("║  SENIOR DEV EXECUTOR — Phase 1: Deterministic Prep                   ║");
  console.log("╚══════════════════════════════════════════════════════════════════════╝");
  console.log();
  ...
  console.log();
  console.log("Modules detected:", result.modules_detected.join(", ") || "(none)");
  console.log();
  ...
  console.log("   Attach: context.json, pr-metadata.json, changes-summary.txt");
  console.log();
  ...
  console.log("           typecheck-output.txt, test-output.txt, related-frames.txt");
  console.log();
...
function printRecallContextResult(
  ...
  if (result.frames.length === 0) {
    ...
    for (const frame of result.frames) {
      ...
      console.log(`- Next: ${frame.status_snapshot?.next_action ?? "(none)"}`);
...
function printCaptureFrameResult(result: Awaited<ReturnType<typeof captureSeniorDevFrame>>): void {
  console.log();
  console.log("╔══════════════════════════════════════════════════════════════════════╗");
  console.log("║  SENIOR DEV EXECUTOR — Phase 4: Frame Capture                        ║");
  console.log("╚══════════════════════════════════════════════════════════════════════╝");
  console.log();
  ...
  if (result.success) {
    ...
    const prKeyword = result.frame_payload.keywords?.find((k) => k.startsWith("pr-"));
    if (prKeyword) {
      console.log(`  lex recall --keyword "${prKeyword}"`);
    }
    const devKeyword = result.frame_payload.keywords?.find((k) => k.startsWith("developer:"));
    if (devKeyword) {
    ...
    console.log(`  --reference-point "${result.frame_payload.reference_point}" \\`);
    console.log(`  --summary "${result.frame_payload.summary_caption}" \\`);
    console.log(`  --next "${result.frame_payload.status_snapshot.next_action}" \\`);
    console.log(`  --modules "${result.frame_payload.module_scope.join(",")}" \\`);
    console.log(`  --keywords "${result.frame_payload.keywords?.join(",") ?? ""}" \\`);
