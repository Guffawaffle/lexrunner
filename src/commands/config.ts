...
function resolveConfigWithPrecedence(baseDir: string = process.cwd()): ConfigShowResult {
  ...
  const resolved = Array.from(configMap.values()).sort((a, b) => a.key.localeCompare(b.key));
        ...
        if (opts.json || deps.jsonModeActive()) {
          ...
          for (const configValue of valuesToShow) {
            ...
            if (configValue.overrides) {
              console.log(
                `    ${chalk.gray("Original:")} ${formatValue(configValue.overrides.value)} ${chalk.gray(`(from ${configValue.overrides.source})`)}`
              );
            }
