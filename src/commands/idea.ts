...
import { generateFingerprint, extractFingerprint, injectFingerprint } from '../utils/fingerprint.js';
...
async function runIdeaCommand(options: IdeaOptions): Promise<void> {
	...
	const outputPath = options.output || expandTokens(
...
async function detectCurrentRepo(): Promise<string> {
	...
	try {
		...
		throw new Error('Not a git repository or no origin remote configured. Use --repo flag to specify repository.');
	}
}
