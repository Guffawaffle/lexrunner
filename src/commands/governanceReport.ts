...
function filterLogs(
  ...
  if (opts.workflow) {
    filtered = filtered.filter((l) => l.context.workflowId === opts.workflow);
  }
...
function computeStats(logs: GovernanceComparisonLog[], topN = 10): Stats {
  ...
  const withConstraints = logs.filter(
    (l) => l.lexsona.constraintSet && l.lexsona.constraintSet.constraintCount > 0
  ).length;

  const avgConstraintCount =
    logs.reduce((s, l) => s + (l.lexsona.constraintSet?.constraintCount ?? 0), 0) / total;
  ...
  const noPersonaCount = logs.filter((l) => l.lexsona.personaId === null).length;
  const hasUncertainty =
    offline > 0 || failedCount > 0 || withConfidenceCeiling > 0 || noPersonaCount > 0;
...
function formatText(stats: Stats): string {
  ...
  lines.push(
    `  offline-mode logs: ${stats.offline} (${((stats.offline / stats.total) * 100).toFixed(0)}%)`
  );
  lines.push(
  ...
  lines.push(`  average constraintCount: ${stats.avgConstraintCount.toFixed(2)}`);
  lines.push(`  persona usage: ${JSON.stringify(stats.personaCounts, null, 2)}`);
  ...
  lines.push(
    `\nSimple agreement: runner allowed and LexSona no-block == ${
      stats.agreeCount
    }/${stats.total} (${((stats.agreeCount / stats.total) * 100).toFixed(0)}%)`
  );
  ...
  if (stats.uncertainty.hasUncertainty) {
    ...
    if (stats.uncertainty.offlineCount > 0) {
      lines.push(`  - offline mode: ${stats.uncertainty.offlineCount} logs (LexSona unavailable)`);
    }
    if (stats.uncertainty.failedCount > 0) {
      lines.push(`  - derivation failures: ${stats.uncertainty.failedCount} logs`);
    }
    ...
    if (stats.uncertainty.noPersonaCount > 0) {
      lines.push(`  - no persona: ${stats.uncertainty.noPersonaCount} logs (persona not set)`);
    }
    lines.push("\n  → These represent areas where governance insight is incomplete.");
    lines.push("    Consider investigating patterns in failed/offline logs.");
  } else {
...
function formatMarkdown(stats: Stats): string {
  ...
  lines.push(
    `| Offline Mode Logs | ${stats.offline} (${((stats.offline / stats.total) * 100).toFixed(
      0
    )}%) |`
  );
  lines.push(
  ...
  lines.push(`| Avg Constraint Count | ${stats.avgConstraintCount.toFixed(2)} |`);
  lines.push(
  ...
  lines.push("\n## Uncertainty Summary\n");
  if (stats.uncertainty.hasUncertainty) {
    lines.push("> ⚠️ **Fail-forward:** Some logs have incomplete governance insight.\n");
    ...
    if (stats.uncertainty.failedCount > 0) {
      lines.push(`| Derivation failures | ${stats.uncertainty.failedCount} |`);
    }
    if (stats.uncertainty.withConfidenceCeiling > 0) {
      lines.push(`| Confidence ceiling | ${stats.uncertainty.withConfidenceCeiling} |`);
    }
    if (stats.uncertainty.noPersonaCount > 0) {
      lines.push(`| No persona set | ${stats.uncertainty.noPersonaCount} |`);
    }
...
function formatTable(stats: Stats): string {
  ...
  lines.push("│ Metric                      │ Value    │");
  lines.push("├─────────────────────────────┼──────────┤");
  lines.push(`│ Total Logs                  │ ${String(stats.total).padStart(8)} │`);
  lines.push(`│ Successful Derivations      │ ${String(stats.successes).padStart(8)} │`);
  lines.push(`│ Offline Mode Logs           │ ${String(stats.offline).padStart(8)} │`);
  lines.push(`│ Logs with Constraints       │ ${String(stats.withConstraints).padStart(8)} │`);
  lines.push(
    `│ Avg Constraint Count        │ ${stats.avgConstraintCount.toFixed(2).padStart(8)} │`
  );
  lines.push(`│ Agreement Rate              │ ${String(stats.agreeCount).padStart(8)} │`);
  lines.push("└─────────────────────────────┴──────────┘");
  ...
  if (Object.keys(stats.personaCounts).length > 0) {
    ...
    lines.push("├──────────────────────────────────────┼───────┤");
    for (const [persona, count] of Object.entries(stats.personaCounts)) {
      lines.push(`│ ${persona.padEnd(36)} │ ${String(count).padStart(5)} │`);
    }
    lines.push("└──────────────────────────────────────┴───────┘");
  }
  ...
  if (stats.uncertainty.hasUncertainty) {
    ...
    if (stats.uncertainty.offlineCount > 0) {
      lines.push(
        `│ Offline mode                         │ ${String(stats.uncertainty.offlineCount).padStart(
          5
        )} │`
      );
    }
    if (stats.uncertainty.failedCount > 0) {
      lines.push(
        `│ Derivation failures                  │ ${String(stats.uncertainty.failedCount).padStart(
          5
        )} │`
      );
    }
...
export async function governanceReport(opts: ReportOptions): Promise<void> {
  ...
  if (opts.acceptLegacy) {
    ...
    normalizationWarningCount = results.reduce((sum, r) => sum + r.normalizationWarnings.length, 0);
    if (legacyCount > 0) {
      ...
      warn("   Legacy logs may have incomplete data. Consider re-running with current schema.\n");
    }
  } else {
  ...
  if (allLogs.length === 0) {
    ...
    if (opts.acceptLegacy) {
      console.log("No governance logs found (even with legacy inclusion).");
    } else {
      console.log(
...
export function registerGovernanceReportCommand(program: Command): void {
  program
    ...
    .option("--disagreements-only", "Show only logs where runner/LexSona disagree")
    .option("--format <type>", "Output format: text, json, table, markdown", "text")
    ...
    .action(async (options) => {
      await governanceReport({
        ...
        format: options.format as "text" | "json" | "table" | "markdown",
