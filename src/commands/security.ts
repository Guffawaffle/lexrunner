...
function createResult<TFindings>(
  command: string,
  status: "ok" | "findings" | "error",
  report: string,
  findings?: TFindings
): SecurityCommandResult<TFindings> {
  let exitCode = 0;
...
export async function checkRotation(
  ...
  SecurityCommandResult<{ summary: { needsRotation: string[]; ok: string[]; maxAgeDays: number } }>
> {
  ...
  for (const secretId of secretIds) {
    try {
      ...
      if (needs) needsRotation.push(secretId);
      else ok.push(secretId);
    } catch (e) {
...
export async function scanPlan(
  planPath: string
): Promise<SecurityCommandResult<{ detected: number; items: any[] }>> {
  ...
  try {
    ...
    return createResult("scan-plan", "findings", report, {
      detected: detected.length,
      items: detected,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return createResult("scan-plan", "error", `Scan failed: ${message}`, {
      detected: -1,
      items: [],
    });
  }
...
export async function validateSecrets(
  secretIds: string[]
): Promise<SecurityCommandResult<{ missing: string[]; present: string[] }>> {
  const secretsManager = new SecretsManager();
