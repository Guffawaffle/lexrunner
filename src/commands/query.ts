...
export class PlanQueryEngine {
  ...
  query(queryString: string): QueryResult {
    ...
    const filteredItems = enrichedItems.filter((item) => this.matchesFilters(item, filters));
  ...
  byLevel(level: number): QueryResult {
    ...
    const enrichedItems = this.enrichItems().filter((item) => items.includes(item.name));
   ...
   */
  roots(): QueryResult {
    const enrichedItems = this.enrichItems().filter((item) => item.deps.length === 0);
  ...
  leaves(): QueryResult {
    ...
    const enrichedItems = this.enrichItems().filter((item) => !hasDependents.has(item.name));
  ...
  private getDependents(itemName: string): string[] {
    return this.plan.items.filter((item) => item.deps.includes(itemName)).map((item) => item.name);
  }
    ...
    .action(async (file: string | undefined, queryString: string | undefined, opts: any) => {
      ...
      try {
        ...
        if (opts.stats) {
          ...
          console.error(
            "Error: query string or option (--stats, --roots, --leaves, --level) required"
          );
          throwExit(1);
        }
