/**
 * Orchestrate commands - Issue analysis and parallel work planning
 */
...
function formatTable(result: IssueAnalysisResult): string {
	...
	lines.push('-'.repeat(80));
	lines.push(
		`${'#'.padEnd(6)} ${'Title'.padEnd(40)} ${'Files'.padEnd(6)} ${'Complexity'.padEnd(10)} ${'Duration'.padEnd(10)}`
	);
	lines.push('-'.repeat(80));
	...
	for (const issue of result.metadata) {
		...
		lines.push(
			`${('#' + issue.number).padEnd(6)} ${title.padEnd(40)} ${files.padEnd(6)} ${complexity.padEnd(10)} ${duration.padEnd(10)}`
		);
	}
	...
	if (highOverlap.length > 0) {
		...
		lines.push('-'.repeat(80));
		lines.push(
			`${'Issue 1'.padEnd(10)} ${'Issue 2'.padEnd(10)} ${'Score'.padEnd(10)} ${'Files'.padEnd(10)} ${'Dirs'.padEnd(10)} ${'Labels'.padEnd(10)}`
		);
		lines.push('-'.repeat(80));
		
		for (const overlap of highOverlap.slice(0, 10)) {
			lines.push(
				`${'#' + overlap.issue1.toString().padEnd(9)} ${'#' + overlap.issue2.toString().padEnd(9)} ${(overlap.score * 100).toFixed(1).padEnd(9) + '%'} ${(overlap.fileOverlap * 100).toFixed(1).padEnd(9) + '%'} ${(overlap.directoryOverlap * 100).toFixed(1).padEnd(9) + '%'} ${(overlap.labelOverlap * 100).toFixed(1).padEnd(9) + '%'}`
			);
		}
	...
	if (result.parallelGroups.length > 0) {
		...
		for (let i = 0; i < result.parallelGroups.length; i++) {
			...
			lines.push(
				`Group ${i + 1}: ${group.issues.map(n => '#' + n).join(', ')} (avg overlap: ${(group.avgOverlap * 100).toFixed(1)}%)`
			);
		}
		lines.push('');
	}
 ...
 */
export function registerOrchestrateCommands(program: Command, jsonModeActive: () => boolean): void {
	const orchestrate = program
		.command('orchestrate')
		.description('Orchestration tools for issue analysis and parallel work planning');

	orchestrate
		.command('analyze')
		.description('Analyze GitHub issues for parallelization and orchestration')
