...
export function parseSignerOption(signerOption: string): SigningOptions {
  ...
  if (colonIndex === -1) {
    throw new Error(
      `Invalid signer option format: ${signerOption}. Expected "kms:<keyRef>" or "gpg:<fingerprint>"`
    );
  }
...
async function signWithGPG(
  ...
  try {
    ...
    const gpgArgs = ["--detach-sign", "--armor", "--local-user", fingerprint, "--output", sigPath];
    ...
    // Export public key
    await execFileAsync("gpg", ["--armor", "--export", fingerprint], {
      encoding: "utf8",
 ...
 * Sign audit manifest
 */
export async function signManifest(manifestPath: string, options: SigningOptions): Promise<void> {
 ...
 */
async function verifyGPGSignature(manifestPath: string, sigPath: string): Promise<boolean> {
  try {
    // Verify signature using GPG
    await execFileAsync("gpg", ["--verify", sigPath, manifestPath]);
    return true;
  } catch (error: any) {
 ...
 */
export async function verifyManifestSignature(manifestPath: string): Promise<VerificationResult> {
  try {
    // Read manifest
    const manifestContent = readFileSync(manifestPath, "utf-8");
    const manifest = JSON.parse(manifestContent);
