...
export function validateAuditEvent(event: any): { valid: boolean; errors?: string[] } {
	...
	if (event.schema_version && typeof event.schema_version === 'string') {
		...
		if (!versionRegex.test(event.schema_version)) {
			errors.push(`Invalid schema_version format: ${event.schema_version} (expected: MAJOR.MINOR.PATCH)`);
		}
	}
 ...
 */
export function parseSchemaVersion(version: string): { major: number; minor: number; patch: number } | null {
